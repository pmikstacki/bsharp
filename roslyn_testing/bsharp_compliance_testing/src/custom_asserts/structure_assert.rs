...
fn normalize_expected_root(
    root: &ExpectedNode,
    actual_global_count: usize,
    apply: bool,
) -> ExpectedNode {
    ...
    for ch in &root.children {
        if apply && ch.kind == "ClassDeclaration" && !harness_replaced {
            let count = if actual_global_count == 0 {
                1
            } else {
                actual_global_count
            };
    ...
    match u {
        UD::Namespace { namespace } => {
            children.push(ExpectedNode {
                ...
                token_value: Some(namespace.to_string()),
                children: vec![],
            });
        }
        UD::Alias {
            ...
            children.push(ExpectedNode {
                kind: "IdentifierName".to_string(),
                token_value: Some(namespace_or_type.to_string()),
                children: vec![],
            });
        }
        UD::Static { type_name } => {
            children.push(ExpectedNode {
                kind: "IdentifierName".to_string(),
                token_value: Some(type_name.to_string()),
                children: vec![],
            });
        }
    }
...
pub fn assert_tree(expected: &ExpectedTree, unit: &bsharp_syntax::ast::CompilationUnit) {
    ...
    let actual_global_count = actual
        .root
        .children
        .iter()
        .filter(|c| c.kind == "GlobalStatement")
        .count();
...
fn top_level_decl_to_expected(d: &bsharp_syntax::ast::TopLevelDeclaration) -> Vec<ExpectedNode> {
    ...
    match d {
        ...
        TopLevelDeclaration::Struct(_) => vec![ExpectedNode {
            kind: "StructDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        TopLevelDeclaration::Interface(_) => vec![ExpectedNode {
            kind: "InterfaceDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        TopLevelDeclaration::Record(_) => vec![ExpectedNode {
            kind: "RecordDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        TopLevelDeclaration::Enum(_) => vec![ExpectedNode {
            kind: "EnumDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        TopLevelDeclaration::Delegate(_) => vec![ExpectedNode {
...
fn ns_to_expected(
    ns: &bsharp_syntax::declarations::FileScopedNamespaceDeclaration,
) -> ExpectedNode {
    let mut children = Vec::new();
    children.push(ExpectedNode {
        kind: "IdentifierName".to_string(),
        token_value: Some(ns.name.to_string()),
        children: vec![],
    });
...
fn ns_member_to_expected(
    d: &bsharp_syntax::declarations::NamespaceBodyDeclaration,
) -> Vec<ExpectedNode> {
    ...
    match d {
        ...
        NBD::Struct(_) => vec![ExpectedNode {
            kind: "StructDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        NBD::Interface(_) => vec![ExpectedNode {
            kind: "InterfaceDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        NBD::Enum(_) => vec![ExpectedNode {
            kind: "EnumDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        NBD::Delegate(_) => vec![ExpectedNode {
            kind: "DelegateDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
        NBD::Record(_) => vec![ExpectedNode {
            kind: "RecordDeclaration".to_string(),
            token_value: None,
            children: vec![],
        }],
...
fn namespace_to_expected(n: &bsharp_syntax::declarations::NamespaceDeclaration) -> ExpectedNode {
    let mut children = vec![ExpectedNode {
        kind: "IdentifierName".to_string(),
        token_value: Some(n.name.to_string()),
        children: vec![],
    }];
    for m in &n.declarations {
        children.extend(ns_member_to_expected(m));
    }
    ExpectedNode {
...
fn class_to_expected(c: &bsharp_syntax::declarations::ClassDeclaration) -> ExpectedNode {
    ...
    children.push(ExpectedNode {
        kind: "IdentifierName".to_string(),
        token_value: Some(c.name.to_string()),
        children: vec![],
    });
    // Type parameters
    if let Some(tps) = &c.type_parameters {
        ...
        for tp in tps {
            ...
            node_children.push(ExpectedNode {
                kind: "IdentifierName".to_string(),
                token_value: Some(tp.name.to_string()),
                children: vec![],
            });
            tp_children.push(ExpectedNode {
...
fn constraint_clause_to_expected(
    cl: &bsharp_syntax::declarations::TypeParameterConstraintClause,
) -> ExpectedNode {
    let mut children: Vec<ExpectedNode> = Vec::new();
    // Constrained type parameter name
    children.push(ExpectedNode {
        kind: "IdentifierName".to_string(),
        token_value: Some(cl.type_param.to_string()),
        children: vec![],
    });
    // Constraints
...
fn constraint_to_expected(
    c: &bsharp_syntax::declarations::TypeParameterConstraint,
) -> ExpectedNode {
    use bsharp_syntax::declarations::TypeParameterConstraint as TPC;
    match c {
        TPC::ReferenceType => ExpectedNode {
            kind: "ReferenceType".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::ValueType => ExpectedNode {
            kind: "ValueType".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::Unmanaged => ExpectedNode {
            kind: "Unmanaged".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::NotNull => ExpectedNode {
            kind: "NotNull".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::Constructor => ExpectedNode {
            kind: "Constructor".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::AllowsRefStruct => ExpectedNode {
            kind: "AllowsRefStruct".to_string(),
            token_value: None,
            children: vec![],
        },
        TPC::SpecificParameter(id) => ExpectedNode {
            kind: "SpecificType".to_string(),
            token_value: None,
            children: vec![ExpectedNode {
                kind: "IdentifierName".to_string(),
                token_value: Some(id.to_string()),
                children: vec![],
            }],
        },
...
fn compare_nodes(path: &str, expected: &ExpectedNode, actual: &ExpectedNode) {
    ...
    if let Some(tok) = &expected.token_value {
        if actual.token_value.as_deref() != Some(tok.as_str()) {
            panic!(
                "Token mismatch at {} (kind {}): expected {:?}, got {:?}",
                path, expected.kind, expected.token_value, actual.token_value
            );
        }
    }
    ...
    if ec != ac {
        panic!(
            "Children count mismatch at {} (kind {}): expected {}, got {}",
            path, expected.kind, ec, ac
        );
    }
