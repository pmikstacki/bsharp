...
pub fn extract_expected_diagnostics(args: &str) -> Vec<RoslynDiagnosticExpectation> {
    ...
    while i + 11 <= bytes.len() {
        // len("Diagnostic(") == 11
        if bytes[i..].starts_with(b"Diagnostic(")
            && is_ident_boundary(i.checked_sub(1).map(|j| bytes[j]))
        {
            // find matching ) for this Diagnostic(
            let open = i + "Diagnostic".len();
            let Some(open_paren) = next_non_ws(args, open) else {
                break;
            };
            let Some(close_paren) = find_call_closing_paren(args, open_paren) else {
                i += 1;
                continue;
            };
            ...
            while let Some((name, args_start, args_end, next_idx)) = try_parse_chain(args, j) {
                match name {
                    "WithLocation" | "WithSpan" | "WithPosition" => {
                        if let Some((a, b)) = parse_two_usize(&args[args_start..args_end]) {
                            item.span = Some((a, b));
                        }
                    }
                    _ => {}
        ...
        match c {
            ...
            '@' => {
                if j + 1 < bytes.len() && bytes[j + 1] == b'"' {
                    j = skip_verbatim_string(inner, j);
                    continue;
                }
            }
            _ => {}
...
fn try_parse_chain(s: &str, mut i: usize) -> Option<(&str, usize, usize, usize)> {
    ...
    let open_paren = i;
    let Some(close_paren) = find_call_closing_paren(s, open_paren) else {
        return None;
    };
