use crate::generator::{
    scanner::{CallKind, find_next_call},
    string_lexer::{
        extract_first_string, find_call_closing_paren, has_following_nonws_comma_before_paren,
    },
};
use crate::tests_writer::utility::{self, Category, ExtractedTest};

pub fn extract_tests_facade(
    content: &str,
        ...
        if let Some((literal, end_idx)) = extract_first_string(content, hit.start_args) {
            ...
            if has_more_args {
                ...
                if let Some(close_paren_idx) = find_call_closing_paren(content, open_paren_idx) {
                    ...
                    let count =
                        crate::generator::roslyn_args::count_diagnostic_invocations(args_slice);
                    if count > 0 {
...
fn map_kind_to_category(kind: CallKind) -> Category {
    match kind {
        CallKind::UsingTree | CallKind::ParseCompilationUnit | CallKind::ParseTree => {
            Category::Tree
        }
