...
pub fn map_kind(roslyn_kind: &str) -> Option<MatchSpec> {
    ...
    match roslyn_kind {
        ...
        // Declarations
        "NamespaceDeclaration" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "NamespaceDeclaration".into(), token_field: None }),
        "FileScopedNamespaceDeclaration" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "FileScopedNamespaceDeclaration".into(), token_field: None }),
        "ClassDeclaration" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "ClassDeclaration".into(), token_field: None }),
        "RecordStructDeclaration" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "RecordDeclaration".into(), token_field: None }),
        // Type parameters and constraints
        "TypeParameterList" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "TypeParameterList".into(), token_field: None }),
        "TypeParameter" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "TypeParameter".into(), token_field: None }),
        "TypeParameterConstraintClause" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "TypeParameterConstraintClause".into(), token_field: None }),
        // Constraint kinds mapping to our enum variants
        "ClassConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "ReferenceType".into(), token_field: None }),
        "StructConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "ValueType".into(), token_field: None }),
        "UnmanagedConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "Unmanaged".into(), token_field: None }),
        "NotNullConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "NotNull".into(), token_field: None }),
        "ConstructorConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "Constructor".into(), token_field: None }),
        "TypeConstraint" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "SpecificType".into(), token_field: None }),
        // Allows family (Roslyn specific) -> our canonical variant
        "AllowsConstraintClause" => Some(MatchSpec { roslyn_kind: roslyn_kind.into(), our_kind: "AllowsRefStruct".into(), token_field: None }),
        _ => None,
    }
}
...
pub fn translate_expected_tree(t: &crate::generator::structure_dsl::ExpectedTree) -> crate::generator::structure_dsl::ExpectedTree {
    ...
    fn rename_kind(kind: &str) -> String {
        ...
        if let Some(stripped) = kind.strip_suffix("Syntax") { return stripped.to_string(); }
        kind.to_string()
    }
    ...
    fn translate_node(n: &ExpectedNode) -> Option<ExpectedNode> {
        ...
        for ch in &n.children {
            ...
            if let Some(tch) = translate_node(ch) { new_children.push(tch); }
        }

        let mut token_value = n.token_value.clone();
        if kind0 == "IdentifierName" {
            // Lift identifier text if available
            if token_value.is_none() { token_value = lifted_token.clone(); }
        }

        // Apply mapping table where available
        let our_kind = map_kind(&kind0).map(|m| m.our_kind).unwrap_or_else(|| kind0.clone());
        ...
        if our_kind == "ClassDeclaration" {
            if let Some(tv) = lifted_token.clone() {
                out_children.insert(0, ExpectedNode { kind: "IdentifierName".to_string(), token_value: Some(tv), children: vec![] });
            }
        } else if our_kind == "TypeParameter" {
            if let Some(tv) = lifted_token.clone() {
                out_children.push(ExpectedNode { kind: "IdentifierName".to_string(), token_value: Some(tv), children: vec![] });
            }
        ...
        Some(ExpectedNode { kind: our_kind, token_value, children: out_children })
    }
