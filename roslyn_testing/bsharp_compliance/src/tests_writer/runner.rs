...
use crate::generator::{pipeline, emitter, writer, model as new_model};
...
pub fn run(cfg: Config) -> Result<()> {
    ...
    if cfg.structure_mode {
        ...
        eprintln!("[structure] include={:?} exclude={:?}", cfg.include, cfg.exclude);
        ...
        let mut grouped: BTreeMap<String, Vec<structure_dsl::ExtractedStructureTest>> = BTreeMap::new();
        ...
        for entry in WalkDir::new(&cfg.src).into_iter().filter_map(|e| e.ok()) {
            ...
            visited += 1;
            if !codegen::is_included(&include_set, &exclude_set, &rel) { if verbose { eprintln!("[structure] skip: {:?}", rel); } continue; }
            matched += 1;
            if verbose { eprintln!("[structure] file: {:?}", rel); }
            ...
            if verbose { eprintln!("[structure] cases found: {}", cases.len()); }
            for c in &mut cases {
            ...
            if cases.len() > cfg.max_per_file {
                eprintln!("[structure] truncating cases: {} -> {}", cases.len(), cfg.max_per_file);
        ...
        eprintln!("[structure] visited .cs files: {}, matched include: {}", visited, matched);

        for (stem, cases) in grouped {
            let module_name = codegen::sanitize_mod_name(&stem);
            eprintln!("[structure] emit module: {} (stem: {}, cases: {})", module_name, stem, cases.len());
            let contents = crate::generator::structure_emitter::emit_structure_tests_for_group(&module_name, &stem, &cases)?;
            crate::generator::writer::write_group_file(&parsing_dir, &module_name, &stem, &contents)?;
    ...
    if cfg.use_new_emitter {
        for (stem, tests) in grouped {
            ...
            let mapped: Vec<new_model::ExtractedTest> = tests.into_iter().map(|t| map_old_to_new(&t)).collect();
...
fn map_old_to_new(t: &ExtractedTest) -> new_model::ExtractedTest {
    let expected = t
        .expected_diag_count
        .map(|n| new_model::ExpectedDiagnostics { count: n, items: vec![] });
