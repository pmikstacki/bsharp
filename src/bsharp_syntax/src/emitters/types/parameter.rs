...
impl Emit for Parameter {
    fn emit<W: std::fmt::Write>(&self, w:&mut W, cx:&mut EmitCtx)->Result<(),EmitError>{
        ...
        if let Some(m) = &self.modifier { m.emit(w, cx)?; w.write_char(' ')?; }
        self.parameter_type.emit(w, cx)?; w.write_char(' ')?; write!(w, "{}", self.name)?;
        if let Some(def) = &self.default_value { w.write_str(" = ")?; def.emit(w, cx)?; }
        Ok(())
    }
}
