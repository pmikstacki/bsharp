...
impl Emit for ClassDeclaration {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        // Keyword and name
        cx.token(w, "class ")?; write!(w, "{}", self.name)?;
        // Type parameters
        if let Some(tps) = &self.type_parameters {
            cx.token(w, "<")?;
            for (i,tp) in tps.iter().enumerate(){ if i!=0 { cx.token(w, ", ")?; } tp.emit(w, cx)?; }
            cx.token(w, ">")?;
        }
        // Primary constructor parameters (C# 12)
        if let Some(params) = &self.primary_constructor_parameters {
            cx.token(w, "(")?;
            for (i,p) in params.iter().enumerate(){ if i!=0 { cx.token(w, ", ")?; } p.emit(w, cx)?; }
            cx.token(w, ")")?;
        }
        // Base types
        if !self.base_types.is_empty() {
            cx.token(w, " : ")?;
            for (i, bt) in self.base_types.iter().enumerate(){ if i!=0 { cx.token(w, ", ")?; } bt.emit(w, cx)?; }
        }
        if let Some(cs) = &self.constraints { for c in cs { cx.space(w)?; c.emit(w, cx)?; } }
        // Header completed
        cx.trace_event("header_done", &[("has_body", "true".to_string()) , ("allman", "true".to_string())]);
        if self.body_declarations.is_empty() { cx.space(w)?; w.write_str("{ }")?; return Ok(()); }
        cx.nl(w)?;
        cx.write_indent(w)?;
