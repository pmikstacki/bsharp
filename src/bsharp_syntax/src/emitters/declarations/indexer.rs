...
impl Emit for IndexerAccessor {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        for (i, a) in self.attributes.iter().enumerate() {
            ...
            cx.token(w, "[")?; a.emit(w, cx)?; cx.token(w, "]")?;
        }
        if !self.attributes.is_empty() { cx.space(w)?; }
...
impl Emit for IndexerDeclaration {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        for (i, a) in self.attributes.iter().enumerate() { if i != 0 { cx.write_indent(w)?; } cx.token(w, "[")?; a.emit(w, cx)?; cx.token(w, "]")?; cx.nl(w)?; }
        if !self.attributes.is_empty() { cx.write_indent(w)?; }
        // Modifiers and signature
        for (i, m) in self.modifiers.iter().enumerate(){ if i!=0 { cx.space(w)?; } m.emit(w, cx)?; }
        if !self.modifiers.is_empty() { cx.space(w)?; }
        self.indexer_type.emit(w, cx)?; cx.token(w, " this[")?;
            ...
            match &get.body {
                Some(b) => {
                    ...
                    if let crate::statements::statement::Statement::Block(stmts) = b {
                        if stmts.len() == 1 {
                            cx.space(w)?; w.write_str("{ ")?; stmts[0].emit(w, cx)?; w.write_str(" }")?;
