    ...
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        match self {
            PropertyAccessor::Get { modifiers, attributes, body } => {
                for al in attributes { al.emit(w, cx)?; cx.space(w)?; }
                for (i, m) in modifiers.iter().enumerate(){ if i!=0 { cx.space(w)?; } m.emit(w, cx)?; }
                if !modifiers.is_empty() { cx.space(w)?; }
                cx.token(w, "get")?;
                match body {
                    Some(stmt) => {
                        cx.space(w)?;
                        if let crate::statements::statement::Statement::Block(stmts) = stmt {
                            if stmts.is_empty() {
                                ...
                                w.write_str("{ ")?; stmts[0].emit(w, cx)?; w.write_str(" }")?;
                            } else {
                                stmt.emit(w, cx)?;
                            }
                        } else { stmt.emit(w, cx)?; }
...
impl Emit for PropertyDeclaration {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        self.property_type.emit(w, cx)?; cx.space(w)?; write!(w, "{}", self.name)?;
        ...
        if trivial {
            ...
            cx.trace_event("header_done", &[("has_body", "true".to_string()), ("allman", "false".to_string())]);
            cx.token(w, " { ")?;
            ...
            cx.token(w, " }")?;
            if let Some(init) = &self.initializer { cx.token(w, " = ")?; init.emit(w, cx)?; cx.token(w, ";")?; }
            Ok(())
        } else {
            ...
            cx.nl(w)?; cx.write_indent(w)?; cx.trace_event("before_open_brace", &[("has_body", "true".to_string()), ("allman", "true".to_string())]); cx.open_brace(w)?;
