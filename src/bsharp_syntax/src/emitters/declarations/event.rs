...
impl Emit for EventAccessorList {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        if let Some(add) = &self.add_accessor {
            cx.write_indent(w)?;
            for al in &add.attributes {
                al.emit(w, cx)?;
                cx.space(w)?;
            }
            for (i, m) in add.modifiers.iter().enumerate() {
                if i != 0 {
                    cx.space(w)?;
                }
                m.emit(w, cx)?;
            }
            if !add.modifiers.is_empty() || !add.attributes.is_empty() {
                cx.space(w)?;
            }
            cx.token(w, "add")?;
            match &add.body {
        ...
        if let Some(remove) = &self.remove_accessor {
            cx.write_indent(w)?;
            for al in &remove.attributes {
                al.emit(w, cx)?;
                cx.space(w)?;
            }
            for (i, m) in remove.modifiers.iter().enumerate() {
                if i != 0 {
                    cx.space(w)?;
                }
                m.emit(w, cx)?;
            }
            if !remove.modifiers.is_empty() || !remove.attributes.is_empty() {
                cx.space(w)?;
            }
            cx.token(w, "remove")?;
            match &remove.body {
