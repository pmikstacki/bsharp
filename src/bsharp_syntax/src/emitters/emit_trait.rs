...
impl EmitCtx {
    ...
    pub fn write_indent<W: Write>(&mut self, w: &mut W) -> Result<(), EmitError> {
        ...
        if self.instrument {
            self.push_tail_bytes(&vec![b' '; 4 * self.indent]);
            self.log_action("write_indent", &[("spaces", (4 * self.indent).to_string())]);
        }
        Ok(())
    }
    ...
    pub fn token<W: Write>(&mut self, w: &mut W, s: &str) -> Result<(), EmitError> {
        w.write_str(s)?;
        if self.instrument {
            self.push_tail_bytes(s.as_bytes());
            self.log_action("token", &[("value", s.to_string())]);
        }
        Ok(())
    }

    // Formatting policy convenience helpers
    pub fn between_header_and_body_of_file<W: Write>(
        &mut self,
        w: &mut W,
    ) -> Result<(), EmitError> {
        ...
        if self.instrument {
            self.log_action(
                "policy",
                &[("name", "between_header_and_body_of_file".to_string())],
            );
        }
        policy::between_header_and_body_of_file(&mut self.clone_for_policy(), w)
    }

    pub fn after_file_scoped_namespace_header<W: Write>(
        &mut self,
        w: &mut W,
    ) -> Result<(), EmitError> {
        if self.instrument {
            self.log_action(
                "policy",
                &[("name", "after_file_scoped_namespace_header".to_string())],
            );
        }
        policy::after_file_scoped_namespace_header(&mut self.clone_for_policy(), w)
    }

    pub fn between_using_blocks_and_declarations<W: Write>(
        &mut self,
        w: &mut W,
    ) -> Result<(), EmitError> {
        if self.instrument {
            self.log_action(
                "policy",
                &[("name", "between_using_blocks_and_declarations".to_string())],
            );
        }
    ...
    pub fn between_top_level_declarations<W: Write>(&mut self, w: &mut W) -> Result<(), EmitError> {
        if self.instrument {
            self.log_action(
                "policy",
                &[("name", "between_top_level_declarations".to_string())],
            );
        }
    ...
    pub fn between_members<W: Write>(&mut self, w: &mut W) -> Result<(), EmitError> {
        ...
        if self.instrument {
            self.log_action(
                "policy",
                &[
                    ("name", "between_members".to_string()),
                    (
                        "blank_line_between_members",
                        self.policy_blank_line_between_members.to_string(),
                    ),
                ],
            );
        }
    ...
    pub fn between_block_items<W: Write>(
        ...
        if self.instrument {
            self.log_action("policy", &[("name", "between_block_items".to_string())]);
        }
    ...
    fn clone_for_policy(&self) -> EmitCtx {
        EmitCtx {
            indent: self.indent,
            policy_blank_line_between_members: self.policy_blank_line_between_members,
        ...
        F: FnMut(&mut W) -> Result<(), EmitError>,
    {
        w.write_char(open)?;
        if self.instrument {
            self.push_tail_bytes(&[open as u8]);
            self.log_action("token", &[("value", open.to_string())]);
        }
        inner(w)?;
        w.write_char(close)?;
        if self.instrument {
            self.push_tail_bytes(&[close as u8]);
            self.log_action("token", &[("value", close.to_string())]);
        }
        Ok(())
    }
    ...
    pub fn open_brace<W: Write>(&mut self, w: &mut W) -> Result<(), EmitError> {
        ...
        if self.instrument {
            self.log_action(
                "open_brace",
                &[
                    ("indent_before", indent_before.to_string()),
                    ("indent_after", self.indent.to_string()),
                ],
            );
        }
        Ok(())
    ...
    pub fn close_brace<W: Write>(&mut self, w: &mut W) -> Result<(), EmitError> {
        ...
        if self.instrument {
            ...
            self.log_action(
                "close_brace",
                &[("indent_after_pop", indent_after_pop.to_string())],
            );
        }
        Ok(())
    }
    ...
    pub fn trace_event(&mut self, action: &str, details: &[(&str, String)]) {
    ...
    fn log_action(&mut self, action: &str, details: &[(&str, String)]) {
        ...
        for (k, v) in details {
            det.insert((*k).to_string(), serde_json::Value::String(v.clone()));
        }
...
impl Emitter {
    ...
    pub fn write_with_ctx<T: Emit>(&self, item: &T, cx: &mut EmitCtx) -> Result<String, EmitError> {
        ...
        let mut iw = InstrumentedWriter {
            out: &mut s,
            cx: cx as *mut EmitCtx,
        };
        item.emit(&mut iw, cx)?;
        Ok(s)
    }
