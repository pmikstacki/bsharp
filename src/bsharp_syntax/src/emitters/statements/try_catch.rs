...
use crate::statements::{CatchClause, FinallyClause, TryStatement};
...
impl Emit for TryStatement {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        cx.nl(w)?;
        cx.write_indent(w)?;
        self.try_block.emit(w, cx)?;
        for c in &self.catches {
            cx.nl(w)?;
            cx.write_indent(w)?;
            c.emit(w, cx)?;
        }
        if let Some(f) = &self.finally_clause {
            cx.nl(w)?;
            cx.write_indent(w)?;
            f.emit(w, cx)?;
        }
...
impl Emit for CatchClause {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        if self.exception_type.is_some() || self.exception_variable.is_some() {
            w.write_char(' ')?;
            w.write_char('(')?;
            if let Some(ty) = &self.exception_type {
                write!(w, "{}", ty)?;
            }
            if let Some(var) = &self.exception_variable {
