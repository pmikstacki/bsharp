...
use crate::statements::{
    CheckedStatement, FixedStatement, ForInitializer, LocalFunctionStatement, LockStatement,
    UncheckedStatement, UnsafeStatement,
};
...
impl Emit for FixedStatement {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        w.write_char(' ')?;
        write!(w, "{} = ", self.var_name)?;
        self.initializer.emit(w, cx)?;
        w.write_str(") ")?;
        self.body.emit(w, cx)
    }
}
