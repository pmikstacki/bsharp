...
use crate::expressions::{LambdaBody, LambdaExpression, LambdaParameter, LambdaParameterModifier};
...
impl Emit for LambdaExpression {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, cx: &mut EmitCtx) -> Result<(), EmitError> {
        ...
        if self.parameters.len() == 1
            && self.parameters[0].ty.is_none()
            && self.parameters[0].modifier.is_none()
        {
...
impl Emit for LambdaParameter {
    fn emit<W: std::fmt::Write>(&self, w: &mut W, _cx: &mut EmitCtx) -> Result<(), EmitError> {
        if let Some(m) = &self.modifier {
            match m {
                LambdaParameterModifier::Ref => w.write_str("ref ")?,
                LambdaParameterModifier::Out => w.write_str("out ")?,
                LambdaParameterModifier::In => w.write_str("in ")?,
            }
        }
