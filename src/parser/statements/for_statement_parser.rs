...
use crate::parser::declarations::variable_declaration_parser::parse_variable_declarator;
        ...
        alt((
            ...
            map(
                tuple((
                    ...
                    context("optional const modifier (expected 'const' or type name)", opt(keyword("const"))),
                    // Parse type name
                    context("variable type (expected valid C# type)", bws(parse_type_expression)),
                    // Parse declarators (name and initializer)
                    context("variable declarators (expected variable name and optional initializer)", separated_list1(bws(bchar(',')), parse_variable_declarator))
                )),
            ...
            map(
                context("expression list (expected comma-separated expressions)", separated_list1(bws(bchar(',')), bws(parse_expression))),
...
pub fn parse_for_statement(input: &str) -> BResult<&str, Statement> {
    context(
        ...
        |input| {
            ...
            let (input, _) = context("opening parenthesis after for (expected '(')", bws(bchar('(')))(input)?;
            
            // 1. Parse initializer (optional)
            let (input, initializer) = context("for loop initializer section (expected variable declaration or expressions, followed by ';')", opt(bws(parse_for_initializer)))(input)?;
            let (input, _) = context("semicolon after for initializer (expected ';')", bws(bchar(';')))(input)?;
            
            // 2. Parse condition (optional)
            let (input, condition) = context("for loop condition section (expected boolean expression or empty, followed by ';')", opt(bws(parse_expression)))(input)?;
            let (input, _) = context("semicolon after for condition (expected ';')", bws(bchar(';')))(input)?;
            
            // 3. Parse iterators - the critical part for the test case
            // We need to handle multiple comma-separated expressions
            let (input, iterators) = context("for loop iterator section (expected comma-separated expressions)", separated_list0(bws(bchar(',')), bws(parse_expression)))(input)?;
            
            // 4. Parse closing parenthesis
            let (input, _) = context("closing parenthesis after for header (expected ')')", cut(bws(bchar(')'))))(input)?;
            
            // 5. Parse body statement, ensuring we handle comments correctly
            let (input, body) = context("for loop body (expected valid C# statement)", bws(parse_statement_ws))(input)?;
