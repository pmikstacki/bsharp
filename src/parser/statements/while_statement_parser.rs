    ...
    context(
        ...
        map(
            tuple((
                ...
                bws(delimited(
                    context("opening parenthesis for while condition (expected '(')", bchar('(')), 
                    context("while loop condition (expected boolean expression)", bws(parse_expression)), 
                    cut(context("closing parenthesis for while condition (expected ')')", bchar(')')))
                )),
                context("while loop body (expected valid C# statement)", bws(parse_statement_ws))
            )),
