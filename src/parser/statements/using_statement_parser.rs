    ...
    context(
        ...
        map(
            tuple((
                ...
                context("opening parenthesis after using (expected '(')", bws(bchar('('))),
                // 3. Resource expression (could be declaration or variable)
                context("resource expression (expected variable declaration or disposable expression)", bws(parse_expression)),
                // 4. Closing parenthesis
                context("closing parenthesis after resource (expected ')')", cut(bws(bchar(')')))),
                // 5. Body statement
                context("using statement body (expected valid C# statement)", parse_statement_ws)
            )),
