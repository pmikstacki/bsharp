...
use crate::parser::declarations::type_declaration_helpers::{parse_close_brace, parse_open_brace};
...
fn parse_enum_members<'a>(input: &'a str) -> BResult<&'a str, Vec<EnumMember>> {
    context(
        ...
        |input: &'a str| {
            trace!("[DEBUG] parse_enum_members: input = {:?}", &input[..std::cmp::min(100, input.len())]);
            ...
            match &result {
                Ok((rest, members)) => {
                    trace!("[DEBUG] parse_enum_members: SUCCESS, {} members found. Remaining: {:?}", members.len(), &rest[..std::cmp::min(50, rest.len())]);
                },
                Err(e) => {
...
fn parse_enum_member<'a>(input: &'a str) -> BResult<&'a str, EnumMember> {
    context(
        ...
        |input: &'a str| {
            trace!("[DEBUG] parse_enum_member: input = {:?}", &input[..std::cmp::min(100, input.len())]);
            ...
            trace!("[DEBUG] parse_enum_member: parsed value = {:?}, final_input for member: {:?}", value.is_some(), &final_input[..std::cmp::min(50, final_input.len())]);

            Ok((final_input, EnumMember { // Use final_input as the remainder
                attributes: attribute_lists,
                name,
                value,
            }))
