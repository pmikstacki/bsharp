...
use crate::syntax::parser_helpers::{context, bws, keyword, bchar, bseparated_list0};
...
pub fn parse_indexer_declaration(input: &str) -> BResult<&str, IndexerDeclaration> {
    context(
        ...
        |input| {
            ...
            let (input, _) = context(
                "indexer opening bracket (expected '[' to start parameter list)",
                bws(bchar('[')),
            )(input)?;
            let (input, parameters) = parse_indexer_parameters(input)?;
            let (input, _) = context(
                "indexer closing bracket (expected ']' to end parameter list)",
                cut(bws(bchar(']'))),
            )(input)?;
...
fn parse_indexer_parameters(input: &str) -> BResult<&str, Vec<Parameter>> {
    use crate::parser::declarations::parameter_parser::parse_parameter;
