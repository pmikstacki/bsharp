...
use crate::syntax::nodes::declarations::{
    Attribute, ClassBodyDeclaration, ClassDeclaration, ConstructorDeclaration, EventDeclaration, IndexerDeclaration, InterfaceBodyDeclaration, 
    InterfaceDeclaration, MethodDeclaration, Modifier, PropertyAccessor, PropertyDeclaration, 
    RecordDeclaration, StructBodyDeclaration, StructDeclaration, TypeDeclaration
};
...
use crate::parser::declarations::destructor_declaration_parser::parse_destructor_declaration;
use crate::parser::declarations::enum_declaration_parser::parse_enum_declaration;
use crate::parser::declarations::event_declaration_parser::parse_event_declaration;
use crate::parser::declarations::field_declaration_parser::parse_field_declaration;
use crate::parser::declarations::indexer_declaration_parser::parse_indexer_declaration;
use crate::parser::declarations::method_declaration_parser::parse_member_declaration;
use crate::parser::declarations::modifier_parser::parse_modifiers;
use crate::parser::declarations::operator_declaration_parser::parse_operator_declaration;
use crate::parser::declarations::parameter_parser::parse_parameter_list;
use crate::parser::declarations::property_declaration_parser::parse_property_declaration;
use crate::parser::declarations::type_declaration_helpers::{at_end_of_body, parse_close_brace};
use crate::parser::declarations::type_parameter_parser::opt_parse_type_parameter_list;
use crate::parser::identifier_parser::parse_identifier;

pub use crate::parser::declarations::modifier_parser::parse_modifiers_for_decl_type;
...
pub fn parse_declaration_header<'a>(input: &'a str, declaration_keyword: &'a str) -> BResult<&'a str, DeclarationHeader<'a>> {
    context(
        ...
        |input| {
            ...
            let (input, _) = tag(declaration_keyword)(input)?;
    ...
    F: Fn(&str) -> BResult<&str, M>,
{
    ...
    while !at_end_of_body(current) {
        ...
        let (after_ws, _) = crate::syntax::comment_parser::parse_whitespace_or_comments(current)?;
        current = after_ws;
...
/// Skip malformed parser until we find a recovery point (semicolon, brace, or end of input)
    ...
    alt((
        ...
        map(
            tuple((
                ...
                opt(bws(nom::character::complete::char(';'))),
            )),
...
fn parse_interface_property(input: &str) -> BResult<&str, PropertyDeclaration> {
        ...
        match accessor {
            ...
            PropertyAccessor::Init(Some(_)) => {
                ...
                let error_tree = ErrorTree::Base {
                    location: input,
                    kind: BaseErrorKind::Expected(Expectation::Tag("Interface property accessor cannot have a body")),
                };
    ...
    if property_decl.initializer.is_some() {
        ...
        let error_tree = ErrorTree::Base {
            location: input,
            kind: BaseErrorKind::Expected(Expectation::Tag("Interface property cannot have an initializer")),
        };
        return Err(nom::Err::Failure(error_tree));
    }
...
pub fn parse_interface_event(input: &str) -> BResult<&str, EventDeclaration> {
    ...
    if event_decl.accessor_list.is_some() {
        ...
        let error_tree = ErrorTree::Base {
            location: input,
            kind: BaseErrorKind::Expected(Expectation::Tag("Interface event cannot have explicit add/remove accessors")),
        };
        return Err(nom::Err::Failure(error_tree));
    }
...
pub fn parse_interface_indexer(input: &str) -> BResult<&str, IndexerDeclaration> {
    ...
    if indexer_decl.accessor_list.get_accessor.is_some() ||
       indexer_decl.accessor_list.set_accessor.is_some() {
        ...
        let error_tree = ErrorTree::Base {
            location: input,
            kind: BaseErrorKind::Expected(Expectation::Tag("Interface indexer accessor cannot have a body")),
        };
        return Err(nom::Err::Failure(error_tree));
    }
    ...
    alt((
        ...
        map(|i| {
            ...
            if member_decl.has_constructor_syntax() {
                ...
                let error_tree = ErrorTree::Base {
                    location: i,
                    kind: BaseErrorKind::Expected(Expectation::Tag("interface method (not constructor parser)")),
                };
