    ...
    context(
        ...
        |input| {
            ...
            if input.trim_start().starts_with("static") {
                ...
                let type_name_str = type_name_parts.iter().map(|id| id.name.clone()).collect::<Vec<_>>().join(".");
            ...
            if input.trim_start().starts_with("=") {
                ...
                let target_str = target_parts.iter().map(|id| id.name.clone()).collect::<Vec<_>>().join(".");
                let (input, _) = bws(bchar(';'))(input)?;
                let (input, _) = ws(input)?;
