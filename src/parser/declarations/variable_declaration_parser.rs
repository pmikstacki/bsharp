...
use crate::syntax::nodes::declarations::local_variable_declaration::{LocalVariableDeclaration, VariableDeclarator};
use crate::syntax::parser_helpers::{bchar, context, bws, bseparated_list1, keyword};
...
pub fn parse_variable_declaration(input: &str) -> BResult<&str, LocalVariableDeclaration> {
    ...
    let (input, is_const) = context(
        ...
        nom::combinator::map(
            nom::combinator::opt(bws(keyword("const"))),
            |opt| opt.is_some()
        ),
    )(input)?;
    ...
    let (input, declarators) = context(
        ...
        bseparated_list1(
            bws(bchar(',')),
            bws(parse_variable_declarator),
        ),
    )(input)?;

    Ok((input, LocalVariableDeclaration {
...
pub fn parse_local_variable_declaration_statement(input: &str) -> BResult<&str, crate::syntax::nodes::statements::statement::Statement> {
