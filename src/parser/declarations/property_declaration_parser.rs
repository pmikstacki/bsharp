...
fn parse_get_accessor(input: &str) -> BResult<&str, PropertyAccessor> {
    ...
    let (input, _) = context("get accessor keyword (expected 'get')", bws(tag_no_case("get")))(input)?;
...
fn parse_set_accessor(input: &str) -> BResult<&str, PropertyAccessor> {
    ...
    let (input, _) = context("set accessor keyword (expected 'set')", bws(tag_no_case("set")))(input)?;
...
fn parse_init_accessor(input: &str) -> BResult<&str, PropertyAccessor> {
    ...
    let (input, _) = context("init accessor keyword (expected 'init')", bws(tag_no_case("init")))(input)?;
...
fn parse_property_accessors(input: &str) -> BResult<&str, Vec<PropertyAccessor>> {
    delimited(
        ...
        many0(
            context("property accessor (expected 'get', 'set', or 'init')", alt((
                parse_get_accessor,
                parse_set_accessor,
                parse_init_accessor
            )))
        ),
        context("property accessors closing (expected '}')", cut(bws(bchar('}'))))
    )(input)
}
...
fn parse_property_initializer(input: &str) -> BResult<&str, Option<Expression>> {
    opt(
        preceded(
            ...
            tuple((
                context("property initializer expression (expected valid C# expression)", bws(parse_expression)),
                context("property initializer terminator (expected ';')", bws(bchar(';')))
            ))
        )
    )(input)
...
pub fn parse_property_declaration(input: &str) -> BResult<&str, PropertyDeclaration> {
    ...
    let (input, modifiers) = context("property modifiers (expected valid property modifiers)", |i| parse_modifiers_for_decl_type(i, "property"))(input)?;
    // Consume any additional optional whitespace before the type
    let (input, _) = ws(input)?;
    let (input, ty) = context("property type (expected valid type expression)", parse_type_expression)(input)?;
    let (input, name) = context("property name (expected valid identifier)", bws(parse_identifier))(input)?;
    let (input, accessors) = context("property accessors (expected accessor block with get/set/init)", bws(parse_property_accessors))(input)?;
