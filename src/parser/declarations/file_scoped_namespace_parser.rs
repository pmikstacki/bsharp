...
use crate::syntax::nodes::declarations::{FileScopedNamespaceDeclaration, namespace_declaration::NamespaceBodyDeclaration};
use crate::syntax::nodes::identifier::Identifier;
use crate::syntax::parser_helpers::{bws, keyword, context, bchar};
...
pub fn parse_file_scoped_namespace_declaration(input: &str) -> BResult<&str, FileScopedNamespaceDeclaration> {
    trace!("[DEBUG] parse_file_scoped_namespace_declaration: input = {:?}", &input[..std::cmp::min(100, input.len())]);
    ...
    trace!("[DEBUG] parse_file_scoped_namespace_declaration: parsed name = {:?}", name);
    ...
    let (input, (using_directives, type_declarations)) = many0(alt((
        ...
        |i| parse_class_declaration(i).map(|(rest, class)| (rest, (None, Some(NamespaceBodyDeclaration::Class(class))))),
        |i| parse_struct_declaration(i).map(|(rest, struct_decl)| (rest, (None, Some(NamespaceBodyDeclaration::Struct(struct_decl))))),
        |i| parse_interface_declaration(i).map(|(rest, interface)| (rest, (None, Some(NamespaceBodyDeclaration::Interface(interface))))),
        |i| parse_enum_declaration(i).map(|(rest, enum_decl)| (rest, (None, Some(NamespaceBodyDeclaration::Enum(enum_decl))))),
        |i| parse_delegate_declaration(i).map(|(rest, delegate)| (rest, (None, Some(NamespaceBodyDeclaration::Delegate(delegate))))),
        |i| parse_record_declaration(i).map(|(rest, record)| (rest, (None, Some(NamespaceBodyDeclaration::Record(record))))),
    )))(input).map(|(rest, items)| {
...
pub fn parse_global_using(input: &str) -> BResult<&str, GlobalUsing> {
    context(
        ...
        |input| {
            ...
            let (input, _) = context(
                "global keyword (expected 'global')",
                bws(keyword("global")),
            )(input)?;
