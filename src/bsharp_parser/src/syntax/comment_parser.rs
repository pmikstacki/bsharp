...
pub fn parse_block_comment<'a>(input: Span<'a>) -> BResult<'a, Span<'a>> {
    recognize((tag("/*"), take_until("*/"), tag("*/"))).parse(input)
}

/// Parse a C# line comment // ... and return the recognized Span
pub fn parse_line_comment<'a>(input: Span<'a>) -> BResult<'a, Span<'a>> {
    recognize((
        tag("//"),
        take_until("\n"),
...
pub fn ws<'a>(input: Span<'a>) -> BResult<'a, &str> {
