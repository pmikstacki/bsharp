    ...
    F: FnMut(&'a str) -> BResult<&'a str, O>,
{
    move |input: &'a str| {
        parser(input).map_err(|err| match err {
            nom::Err::Error(e) => {
                let labeled = ErrorTree::Stack {
                    base: Box::new(e),
                    contexts: vec![(input, StackContext::Context(lbl))],
                };
                nom::Err::Error(labeled)
            }
            nom::Err::Failure(e) => {
                let labeled = ErrorTree::Stack {
                    base: Box::new(e),
...
pub fn bchar(c: char) -> impl Fn(&str) -> BResult<&str, char> {
    move |input: &str| {
        let parsed = char(c).context("character").parse(input);
        if let Ok((remaining, matched)) = parsed.as_ref() {
            if remaining.len() < input.len() {
                brace_tracker::on_char(input, *matched);
            }
        }
        parsed
    }
...
pub fn parse_delimited_list_or_singleton<
    'a,
    FO,
    FF,
    FS,
