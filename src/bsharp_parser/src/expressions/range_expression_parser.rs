...
fn parse_range_starting_with_dots<'a>(input: Span<'a>) -> BResult<'a, Expression> {
    let (input, _) = delimited(ws, tuple((nom_char('.'), nom_char('.'))), ws).parse(input)?;
...
pub(crate) fn parse_range_expression_or_higher<'a>(input: Span<'a>) -> BResult<'a, Expression> {
    nom::branch::alt((
        ...
        (|i| {
            ...
            if let Ok((i_after_dots, _)) = delimited(ws, tuple((nom_char('.'), nom_char('.'))), ws).parse(i) {
