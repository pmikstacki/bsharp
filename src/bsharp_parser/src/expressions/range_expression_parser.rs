...
fn parse_range_starting_with_dots(input: Span) -> BResult<Expression> {
    ...
    let (input, end_operand) =
        opt(delimited(ws, parse_unary_expression_or_higher, ws)).parse(input)?;
    Ok((
        input,
...
pub(crate) fn parse_range_expression_or_higher(input: Span) -> BResult<Expression> {
    nom::branch::alt((
        ...
        (|i| {
            ...
            if let Ok((i_after_dots, _)) =
                delimited(ws, (nom_char('.'), nom_char('.')), ws).parse(i)
            {
                let (i_after_end, end_expr) =
                    opt(delimited(ws, parse_unary_expression_or_higher, ws)).parse(i_after_dots)?;
                Ok((
                    i_after_end,
                    Expression::Range(Box::new(RangeExpression {
