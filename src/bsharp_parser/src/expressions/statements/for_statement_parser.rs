...
pub fn parse_for_statement<'a>(input: Span<'a>) -> BResult<'a, Statement> {
    (|input| {
        ...
        let (input, iterators) = parse_list0(parse_expression, nom_char(','))
            .context("for loop iterators")
            .parse(input)?;

        // 4. Parse closing parenthesis
        let (input, _) = cut(delimited(ws, nom_char(')'), ws))
