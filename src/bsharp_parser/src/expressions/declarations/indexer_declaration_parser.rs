...
use crate::syntax::nodes::declarations::{
    IndexerAccessor, IndexerAccessorList, IndexerDeclaration,
};
...
fn parse_accessors(
    input: &str,
) -> BResult<&str, (Option<IndexerAccessor>, Option<IndexerAccessor>)> {
    context(
            ...
            let (input, body) = alt((
                ...
                map(
                    context(
                        ...
                        nom::combinator::cut(parse_block_statement),
                    ),
                    Some,
                ),
            ))(input)?;
