...
pub fn parse_using_directive(input: Span) -> BResult<UsingDirective> {
    (|input| {
        ...
        let (input_after_static, opt_static) = nom::combinator::opt(delimited(ws, kw_static(), ws)).parse(input.into())?;
        if opt_static.is_some() {
            let (input_after_name, type_name_parts) = delimited(ws, parse_qualified_name, ws).parse(input_after_static.into())?;
            let type_name_segments: Vec<String> = type_name_parts
                ...
                .collect();
            let (input_final, _) = delimited(ws, nom_char(';'), ws).parse(input_after_name.into())?;
        ...
        let (input, left_parts) = delimited(ws, parse_qualified_name, ws).parse(input.into())?;
        ...
        if peek(delimited(ws, tok_assign(), ws))
            ...
            .is_ok()
        {
            let (input, _) = delimited(ws, tok_assign(), ws).parse(input.into())?;
            let (input, right_parts) =
                delimited(ws, parse_qualified_name, ws).parse(input.into())?;
            let right_segments: Vec<String> = right_parts
                ...
                .collect();
            let (input, _) = delimited(ws, nom_char(';'), ws).parse(input.into())?;
