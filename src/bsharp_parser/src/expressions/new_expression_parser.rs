...
pub(crate) fn parse_new_expression(input: Span) -> BResult<Expression> {
    nom::combinator::map(alt((
        ...
        map(
            ...
            |members| {
                Expression::AnonymousObject(AnonymousObjectCreationExpression { initializers: members })
            },
        ),
...
fn object_initializer_guard(input: Span) -> BResult<()> {
    ...
    map(
        alt((
            map(tuple((delimited(ws, parse_identifier, ws), delimited(ws, nom_char('='), ws))), |_| ()),
            map(delimited(ws, nom_char('['), ws), |_| ()),
        )),
        |_| (),
    )
    .parse(input)
...
fn enhanced_collection_initializer(input: Span) -> BResult<InitializerKind> {
    map(
        separated_list0(delimited(ws, nom_char(','), ws), delimited(ws, parse_expression, ws)),
