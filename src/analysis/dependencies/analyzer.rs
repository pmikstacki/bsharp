...
impl DependencyAnalyzer {
    ...
    pub fn add_dependency(&mut self, from: &str, to: &str) {
        ...
        self.dependency_graph
            .add_edge(from_idx, to_idx, DependencyType::Usage);
    }
    ...
    pub fn calculate_module_metrics(&self, module_name: &str) -> DependencyMetrics {
        if let Some(module) = self.modules.get(module_name) {
            ...
            metrics.total_dependencies =
                metrics.outgoing_dependencies + metrics.incoming_dependencies;
    ...
    pub fn check_dependency_inversion_violations(
        &self,
        dependencies: &ClassDependencies,
    ) -> Vec<DependencyInversionViolation> {
        let mut violations = Vec::new();
        ...
        for field_dep in &dependencies.field_dependencies {
            if self.concrete_types.contains(field_dep) {
                violations.push(DependencyInversionViolation {
                    ...
                    suggestion: format!(
                        "Consider depending on an interface instead of {}",
                        field_dep
                    ),
                });
            }
            ...
            for direct in &impact.directly_affected {
                if let Some(transitive_module) = self.modules.get(direct) {
                    impact
                        ...
                        .extend(transitive_module.incoming_dependencies.clone());
                }
            }
    ...
    pub fn calculate_overall_metrics(&self) -> OverallDependencyMetrics {
        ...
        if metrics.total_modules > 0 {
            let total_fan_out: usize = self
                .modules
                .values()
                .map(|m| m.outgoing_dependencies.len())
                .sum();
            let total_fan_in: usize = self
                .modules
                .values()
                .map(|m| m.incoming_dependencies.len())
                .sum();
            ...
            if max_possible_deps > 0 {
                metrics.coupling_density =
                    metrics.total_dependencies as f64 / max_possible_deps as f64;
            }
        }
    ...
    fn analyze_class_member_dependencies(
        &self,
        member: &crate::syntax::nodes::declarations::ClassBodyDeclaration,
        dependencies: &mut ClassDependencies,
    ) {
