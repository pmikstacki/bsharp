use crate::analysis::framework::passes::Phase;
use crate::analysis::framework::registry::AnalyzerRegistry;
use crate::analysis::framework::rules::Rule;
use crate::analysis::framework::session::AnalysisSession;
use crate::analysis::framework::walker::{AstWalker, NodeRef, Visit};
use crate::syntax::ast::CompilationUnit;
use crate::analysis::metrics::MetricsVisitor;
use crate::analysis::report::{AnalysisReport, CfgSummary};
...
impl AnalyzerPipeline {
    ...
    pub fn run_for_file(cu: &CompilationUnit, session: &mut AnalysisSession, registry: &AnalyzerRegistry) {
    ...
    fn run_phase(phase: Phase, cu: &CompilationUnit, session: &mut AnalysisSession, registry: &AnalyzerRegistry) {
    ...
    fn run_local_rules(cu: &CompilationUnit, session: &mut AnalysisSession, registry: &AnalyzerRegistry) {
        ...
        for rs in registry.rulesets_local() {
            ...
            for r in rs.iter() { all_rules.push(r); }
        }
        // Even if there are no rules, we still want to collect metrics in this phase

        struct RulesVisitor<'a> { rules: Vec<&'a Box<dyn Rule>> }
        impl<'a> Visit for RulesVisitor<'a> {
            fn enter(&mut self, node: &NodeRef, session: &mut AnalysisSession) {
                for r in &self.rules { r.visit(node, session); }
            }
        }
    ...
    fn run_semantic_rules(cu: &CompilationUnit, session: &mut AnalysisSession, registry: &AnalyzerRegistry) {
        let mut all_rules: Vec<&Box<dyn Rule>> = Vec::new();
    ...
    pub fn run_workspace(workspace: &Workspace) -> AnalysisReport {
        ...
        for path in files {
            ...
            let Ok((cu, spans)) = parser.parse_with_spans(&source) else { continue };
        ...
        merged_diags.diagnostics.sort_by(|a, b| {
            ...
            af.cmp(&bf)
                .then_with(|| a.location.as_ref().map(|l| l.line).unwrap_or(0).cmp(&b.location.as_ref().map(|l| l.line).unwrap_or(0)))
                .then_with(|| a.location.as_ref().map(|l| l.column).unwrap_or(0).cmp(&b.location.as_ref().map(|l| l.column).unwrap_or(0)))
                .then_with(|| a.code.as_str().cmp(b.code.as_str()))
        });
        // Merge workspace-level loader errors as warnings (deterministic ordering)
        if let Some(sol) = &workspace.solution { ws_warnings.extend(sol.errors.clone()); }
    ...
    pub fn run_workspace_with_config(workspace: &Workspace, config: crate::analysis::context::AnalysisConfig) -> AnalysisReport {
        let mut files: Vec<std::path::PathBuf> = workspace
            .projects
            .iter()
            .flat_map(|p| p.files.iter())
        ...
        for path in files {
            ...
            merged_diags.extend(report.diagnostics.clone());
            if let Some(m) = report.metrics {
                merged_metrics = Some(match merged_metrics.take() { Some(prev) => prev.combine(m), None => m });
            }
            if let Some(cfg) = report.cfg {
                merged_cfg = Some(match merged_cfg.take() {
                    Some(prev) => CfgSummary {
        ...
        #[cfg(feature = "parallel_analysis")]
        {
            let results: Vec<(std::path::PathBuf, AnalysisReport)> = files
                ...
                .filter_map(|path| {
                    ...
                    let parser = Parser::new();
                    let Ok((cu, spans)) = parser.parse_with_spans(&source) else { return None };
                    let mut ctx = AnalysisContext::new(path_str, source);
                    ctx.config = config.clone();
                    let mut session = AnalysisSession::new(ctx.clone(), spans.clone());
                    Self::run_with_defaults(&cu, &mut session);
