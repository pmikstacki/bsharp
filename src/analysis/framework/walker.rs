...
impl<'a> AstWalker<'a> {
    ...
    fn notify_enter(&mut self, node: &NodeRef, session: &mut AnalysisSession) {
        for v in self.visitors.iter_mut() { v.enter(node, session); }
    }

    fn notify_exit(&mut self, node: &NodeRef, session: &mut AnalysisSession) {
        for v in self.visitors.iter_mut().rev() { v.exit(node, session); }
    }
    ...
    fn visit_namespace_member(&mut self, member: &'a NamespaceBodyDeclaration, session: &mut AnalysisSession) {
        match member {
        ...
        for member in &class.body_declarations {
            match member {
                ...
                ClassBodyDeclaration::Constructor(c) => {
                    if let Some(body) = &c.body { self.visit_statement(body, session); }
                }
    ...
    fn visit_method(&mut self, m: &'a crate::syntax::nodes::declarations::MethodDeclaration, session: &mut AnalysisSession) {
        let node = NodeRef::Method(m);
        self.notify_enter(&node, session);
        if let Some(body) = &m.body { self.visit_statement(body, session); }
        self.notify_exit(&node, session);
    }
    ...
    fn visit_statement(&mut self, stmt: &'a Statement, session: &mut AnalysisSession) {
        ...
        match stmt {
            If(s) => {
                ...
                if let Some(alt) = &s.alternative { self.visit_statement(alt, session); }
            }
            For(s) => { self.visit_statement(&s.body, session); }
            While(s) => { self.visit_statement(&s.body, session); }
            DoWhile(s) => { self.visit_statement(&s.body, session); }
            Switch(sw) => {
                for sec in &sw.sections { for st in &sec.statements { self.visit_statement(st, session); } }
            }
            Try(t) => {
                self.visit_statement(&t.try_block, session);
                for c in &t.catches { self.visit_statement(&c.block, session); }
                if let Some(fin) = &t.finally_clause { self.visit_statement(&fin.block, session); }
            }
            Using(u) => { if let Some(body) = &u.body { self.visit_statement(body, session); } }
            Block(stmts) => { for st in stmts { self.visit_statement(st, session); } }
            _ => {}
        }
        self.notify_exit(&node, session);
    }
