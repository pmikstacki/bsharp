use crate::analysis::artifacts::cfg::{ControlFlowIndex, MethodControlFlowStats};
...
use crate::syntax::nodes::declarations::{ClassBodyDeclaration, ClassDeclaration, NamespaceDeclaration, NamespaceBodyDeclaration};
use crate::syntax::nodes::statements::statement::Statement;

pub struct ControlFlowPass;

impl AnalyzerPass for ControlFlowPass {
    fn id(&self) -> &'static str { "passes.control_flow" }
...
fn analyze_compilation_unit(cu: &CompilationUnit, ns_path: Option<String>, index: &mut ControlFlowIndex) {
    for decl in &cu.declarations {
        match decl {
            TopLevelDeclaration::Namespace(ns) => analyze_namespace(ns, ns.name.name.clone(), index),
            TopLevelDeclaration::Class(class) => analyze_class(class, ns_path.as_deref(), &mut Vec::new(), index),
            _ => {}
        }
    }
}
...
fn analyze_namespace(ns: &NamespaceDeclaration, ns_path: String, index: &mut ControlFlowIndex) {
    for member in &ns.declarations {
        match member {
            NamespaceBodyDeclaration::Namespace(inner) => analyze_namespace(inner, format!("{}.{}", ns_path, inner.name.name), index),
            NamespaceBodyDeclaration::Class(class) => analyze_class(class, Some(&ns_path), &mut Vec::new(), index),
...
fn analyze_class(class: &ClassDeclaration, ns_path: Option<&str>, class_stack: &mut Vec<String>, index: &mut ControlFlowIndex) {
...
fn decision_points(stmt: &Statement) -> usize {
    match stmt {
        Statement::If(if_stmt) => {
            ...
            if let Some(alt) = &if_stmt.alternative { d += decision_points(alt); }
            d
        }
        ...
        Statement::Try(try_stmt) => {
            ...
            for h in &try_stmt.catches { d += decision_points(&h.block); }
            if let Some(fin) = &try_stmt.finally_clause { d += decision_points(&fin.block); }
            d
        }
...
fn max_nesting_of(stmt: &Statement, current: usize) -> usize {
    match stmt {
        Statement::If(if_stmt) => {
            ...
            let a = if let Some(alt) = &if_stmt.alternative { max_nesting_of(alt, new_depth) } else { new_depth };
            c.max(a)
        }
        ...
        Statement::Block(stmts) => {
            ...
            for s in stmts { max_d = max_d.max(max_nesting_of(s, current)); }
            max_d
        }
        _ => current,
    }
    ...
    match stmt {
        ...
        Some(s) => match s {
            ...
            Statement::If(if_stmt) => {
                ...
                if let Some(alt) = &if_stmt.alternative { c += count_exit_points(Some(alt)); }
                c
            }
            ...
            Statement::Try(try_stmt) => {
                ...
                for h in &try_stmt.catches { c += count_exit_points(Some(&h.block)); }
                if let Some(fin) = &try_stmt.finally_clause { c += count_exit_points(Some(&fin.block)); }
                c
            }
