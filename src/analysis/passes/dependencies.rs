use crate::analysis::artifacts::dependencies::{DependencyGraph, DependencyNodeType, DependencyType};
use crate::analysis::artifacts::symbols::{SymbolIndex, SymbolId};
...
use crate::syntax::nodes::declarations::{ClassBodyDeclaration, ClassDeclaration, NamespaceBodyDeclaration, NamespaceDeclaration};
...
impl AnalyzerPass for DependenciesPass {
    fn id(&self) -> &'static str { "passes.dependencies" }
...
fn analyze_compilation_unit(cu: &CompilationUnit, symbols: &SymbolIndex, graph: &mut DependencyGraph) {
...
fn analyze_namespace(ns: &NamespaceDeclaration, symbols: &SymbolIndex, graph: &mut DependencyGraph) {
...
fn add_class_dependencies(class: &ClassDeclaration, symbols: &SymbolIndex, graph: &mut DependencyGraph) {
    ...
    for member in &class.body_declarations {
        match member {
            ClassBodyDeclaration::Field(field) => {
                if let Some(target) = type_name(&field.ty) {
                    if let Some(to_id) = resolve_first_id(symbols, &target) {
                        graph.add_node(to_id, DependencyNodeType::Class);
            ...
            ClassBodyDeclaration::NestedClass(nested) => add_class_dependencies(nested, symbols, graph),
            _ => {}
        }
    }
}
...
fn collect_invocations_from_statement(stmt: &crate::syntax::nodes::statements::statement::Statement, sink: &mut impl FnMut(String)) {
    ...
    match stmt {
        ...
        If(if_stmt) => {
            ...
            if let Some(alt) = &if_stmt.alternative { collect_invocations_from_statement(alt, sink); }
        }
        For(for_stmt) => {
            for it in &for_stmt.iterator { collect_invocations_from_expression(it, sink); }
            if let Some(cond) = &for_stmt.condition { collect_invocations_from_expression(cond, sink); }
        ...
        Switch(sw) => {
            ...
            for sec in &sw.sections { for s in &sec.statements { collect_invocations_from_statement(s, sink); } }
        }
        Try(try_stmt) => {
            collect_invocations_from_statement(&try_stmt.try_block, sink);
            for h in &try_stmt.catches { collect_invocations_from_statement(&h.block, sink); }
            if let Some(fin) = &try_stmt.finally_clause { collect_invocations_from_statement(&fin.block, sink); }
        }
        Block(stmts) => { for s in stmts { collect_invocations_from_statement(s, sink); } }
        _ => {}
    }
}
fn collect_invocations_from_expression(expr: &Expression, sink: &mut impl FnMut(String)) {
    match expr {
        &Expression::Invocation(ref inv) => {
            if let Some(name) = invocation_target_name(inv) { sink(name); }
            for arg in &inv.arguments { collect_invocations_from_expression(&arg.expr, sink); }
        }
        &Expression::MemberAccess(ref ma) => {
            collect_invocations_from_expression(&ma.object, sink);
        }
        &Expression::Unary { ref expr, .. } | &Expression::PostfixUnary { ref expr, .. } => {
        ...
        &Expression::Tuple(ref t) => {
            for e in &t.elements { collect_invocations_from_expression(&e.value, sink); }
        }
        &Expression::Index(ref i) => collect_invocations_from_expression(&i.value, sink),
        &Expression::NullConditional(ref nc) => {
            collect_invocations_from_expression(&nc.target, sink);
            if let Some(ref arg) = nc.argument { collect_invocations_from_expression(arg, sink); }
        }
        &Expression::Await(ref a) => collect_invocations_from_expression(&a.expr, sink),
        &Expression::Cast { ref expression, .. } | &Expression::As { ref expression, .. } => {
            collect_invocations_from_expression(expression, sink)
        }
        &Expression::IsPattern { ref expression, .. } => collect_invocations_from_expression(expression, sink),
        &Expression::Nameof(ref n) => collect_invocations_from_expression(&n.expr, sink),
        &Expression::Ref(ref e) => collect_invocations_from_expression(e, sink),
        _ => {}
    }
}
