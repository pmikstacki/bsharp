...
use crate::syntax::nodes::{
    declarations::{StructDeclaration, ClassDeclaration, MethodDeclaration, InterfaceDeclaration, EnumDeclaration, RecordDeclaration, DelegateDeclaration},
    expressions::expression::Expression,
    statements::statement::Statement,
};

impl AstNavigate for StructDeclaration {
...
impl FindDeclarations for StructDeclaration {
    ...
    fn find_structs(&self) -> Vec<&StructDeclaration> { vec![self] }
    ...
    fn find_by_name(&self, name: &str) -> Vec<DeclarationInfo> {
        if self.name.name == name {
            vec![DeclarationInfo { name: self.name.name.clone(), declaration_type: DeclarationType::Struct, location: None }]
        } else { Vec::new() }
    }
}
