...
use crate::syntax::nodes::{
    declarations::{InterfaceDeclaration, ClassDeclaration, MethodDeclaration, StructDeclaration, EnumDeclaration, RecordDeclaration, DelegateDeclaration},
    expressions::expression::Expression,
    statements::statement::Statement,
};

impl AstNavigate for InterfaceDeclaration {
...
impl FindDeclarations for InterfaceDeclaration {
    fn find_classes(&self) -> Vec<&ClassDeclaration> { Vec::new() }
    fn find_methods(&self) -> Vec<&MethodDeclaration> { Vec::new() }
    fn find_interfaces(&self) -> Vec<&InterfaceDeclaration> { vec![self] }
    ...
    fn find_by_name(&self, name: &str) -> Vec<DeclarationInfo> {
        if self.name.name == name {
            vec![DeclarationInfo { name: self.name.name.clone(), declaration_type: DeclarationType::Interface, location: None }]
        } else { Vec::new() }
    }
}
