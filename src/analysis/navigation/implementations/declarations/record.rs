...
use crate::syntax::nodes::{
    declarations::{RecordDeclaration, ClassDeclaration, MethodDeclaration, InterfaceDeclaration, StructDeclaration, EnumDeclaration, DelegateDeclaration},
    expressions::expression::Expression,
    statements::statement::Statement,
};

impl AstNavigate for RecordDeclaration {
...
impl FindDeclarations for RecordDeclaration {
    ...
    fn find_records(&self) -> Vec<&RecordDeclaration> { vec![self] }
    ...
    fn find_by_name(&self, name: &str) -> Vec<DeclarationInfo> {
        if self.name.name == name {
            vec![DeclarationInfo { name: self.name.name.clone(), declaration_type: DeclarationType::Record, location: None }]
        } else { Vec::new() }
    }
}
