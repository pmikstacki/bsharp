...
use crate::syntax::nodes::{
    declarations::{ClassDeclaration, DelegateDeclaration, EnumDeclaration, InterfaceDeclaration, MethodDeclaration, RecordDeclaration, StructDeclaration},
...
impl FindDeclarations for MethodDeclaration {
    ...
    fn find_by_name(&self, name: &str) -> Vec<DeclarationInfo> {
        if self.name.name == name {
            vec![DeclarationInfo { name: self.name.name.clone(), declaration_type: DeclarationType::Method, location: None }]
        } else {
            Vec::new()
        }
    }
