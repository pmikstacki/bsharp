...
fn lookup_span(info: &DeclarationInfo, spans: &SpanTable) -> Option<(usize, usize)> {
    let key = match info.declaration_type {
        ...
        DeclarationType::Method | DeclarationType::Property | DeclarationType::Field | DeclarationType::Event | DeclarationType::Constructor => {
            // Top-level span table currently does not include members.
            return None;
        }
    };

    spans.get(&key).map(|r| (r.start, r.end.saturating_sub(r.start)))
}
