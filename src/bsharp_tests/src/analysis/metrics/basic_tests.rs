...
fn test_collect_from_statement() {
    ...
    let if_stmt = Statement::If(Box::new(IfStatement {
        condition: syntax::nodes::expressions::expression::Expression::Literal(
            syntax::nodes::expressions::literal::Literal::Boolean(true),
        ),
...
fn test_collect_various_statement_types() {
    ...
    let statements = vec![
        Statement::While(Box::new(WhileStatement {
            condition: Box::new(
                syntax::nodes::expressions::expression::Expression::Literal(
                    syntax::nodes::expressions::literal::Literal::Boolean(true),
                ),
            ),
            body: Box::new(Statement::Block(Vec::new())),
        ...
        Statement::Switch(Box::new(SwitchStatement {
            expression: syntax::nodes::expressions::expression::Expression::Literal(
                syntax::nodes::expressions::literal::Literal::Integer(1),
            ),
                ...
                body: Some(Statement::Block(vec![Statement::If(Box::new(
                    IfStatement {
                        ...
                        alternative: Some(Box::new(Statement::While(Box::new(WhileStatement {
                            condition: Box::new(
                                syntax::nodes::expressions::expression::Expression::Literal(
                                    syntax::nodes::expressions::literal::Literal::Boolean(
                                        false,
                                    ),
                                ),
                            ),
...
fn test_collector_integration() {
    ...
    let statements = vec![
        Statement::If(Box::new(IfStatement {
            condition: syntax::expressions::expression::Expression::Literal(
                syntax::expressions::literal::Literal::Boolean(false),
            ),
            consequence: Box::new(Statement::Block(vec![Statement::For(Box::new(
                ForStatement {
                    initializer: None,
        ...
        Statement::While(Box::new(WhileStatement {
            condition: Box::new(
                syntax::expressions::expression::Expression::Literal(
                   syntax::expressions::literal::Literal::Boolean(true),
                ),
            ),
            body: Box::new(Statement::Block(Vec::new())),
        })),
...
fn create_complex_method_for_analysis() -> MethodDeclaration {
    let complex_body = Statement::Block(vec![Statement::If(Box::new(IfStatement {
        ...
        alternative: Some(Box::new(Statement::While(Box::new(WhileStatement {
            condition: Box::new(
                syntax::nodes::expressions::expression::Expression::Literal(
                    syntax::nodes::expressions::literal::Literal::Boolean(false),
                ),
            ),
            body: Box::new(Statement::Block(Vec::new())),
        })))),
