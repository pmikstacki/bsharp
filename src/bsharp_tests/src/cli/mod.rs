...
pub fn tree_execute(input: PathBuf, output: Option<PathBuf>, format: String) -> Result<()> {
    use parser::bsharp::parse_csharp_source;
    use parser::parse_mode;
    use parser::syntax::{ast, nodes::declarations::UsingDirective};
    use nom::Finish;
    ...
    let output_path = output.unwrap_or_else(|| {
        let mut p = input.clone();
        match fmt.as_str() { "dot" | "graphviz" => p.set_extension("dot"), _ => p.set_extension("mmd") };
        p
    });
