...
fn test_single_element_not_a_tuple() {
    let result = parse_expression("(1)".into()).map(|(rest, s)| (rest, s.node));
    assert!(
        result.is_ok(),
    ...
    let result_named = parse_expression("(a: 1)".into()).map(|(rest, s)| (rest, s.node));
    assert!(
        result_named.is_err(),
...
fn test_tuple_missing_closing_paren_is_error() {
    let result = parse_expression("(1, 2".into()).map(|(rest, s)| (rest, s.node));
    assert!(
        result.is_err(),
...
fn test_tuple_invalid_element_name() {
    ...
    let result = parse_expression("(1: x, 2)".into()).map(|(rest, s)| (rest, s.node));
    assert!(
        result.is_err(),
