...
fn parenthesized_literal() {
    let (rest, expr) = parse_expression("(42)".into()).expect("parse ok");
...
fn nested_parentheses() {
    let (rest, expr) = parse_expression("(((true)))".into()).expect("parse ok");
    assert!(rest.fragment().trim().is_empty());
...
fn parenthesized_with_binary_inside() {
    let (rest, expr) = parse_expression("(1 + 2)".into()).expect("parse ok");
    assert!(rest.fragment().trim().is_empty());
    match expr {
        Expression::Binary { .. } => {}
...
fn parenthesized_then_postfix_call() {
    ...
    let (rest, expr) = expect_ok(input, parse_expression(input.into()));
