...
fn test_parse_lambda_with_types() {
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        ...
        if let Some(Type::Primitive(PrimitiveType::Int)) = lambda.parameters[0].ty {
            ...
            panic!(
                "Expected int type for first parameter, got: {:?}",
                lambda.parameters[0].ty
            );
        }
        ...
        if let Some(Type::Primitive(PrimitiveType::String)) = lambda.parameters[1].ty {
            ...
            panic!(
                "Expected string type for second parameter, got: {:?}",
                lambda.parameters[1].ty
            );
        }
    } else {
        panic!("Expected Lambda expression");
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        ...
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                1,
                "Expected 1 statement in lambda block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        ...
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                2,
                "Expected 2 statements in lambda block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        ...
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                2,
                "Expected 2 statements in async lambda block body"
            );
        } else {
            panic!("Expected Block body, got: {:?}", lambda.body);
        }
    ...
    if let Ok(Expression::AnonymousMethod(anon_method)) = result {
        ...
        if let LambdaBody::Block(statements) = &anon_method.body {
            assert_eq!(
                statements.len(),
                2,
                "Expected 2 statements in anonymous method block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                1,
                "Expected 1 statement (if statement) in lambda block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                4,
                "Expected 4 statements in lambda block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                1,
                "Expected 1 statement (try statement) in lambda block body"
            );
    ...
    if let Ok(Expression::Lambda(lambda)) = result {
        if let LambdaBody::Block(statements) = &lambda.body {
            assert_eq!(
                statements.len(),
                1,
                "Expected 1 statement (foreach) in lambda block body"
            );
