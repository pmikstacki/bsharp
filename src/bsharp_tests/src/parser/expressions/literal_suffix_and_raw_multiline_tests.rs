...
fn float_and_decimal_suffixes() {
    match parse_ok("1.0f") {
        Literal::Float(v) => assert!((v - 1.0).abs() < 1e-9),
        other => panic!("expected Float, got {:?}", other),
    }
    match parse_ok("2.5D") {
        Literal::Float(v) => assert!((v - 2.5).abs() < 1e-9),
        other => panic!("expected Float, got {:?}", other),
    }
    match parse_ok("1m") {
        Literal::Decimal(s) => assert_eq!(s, "1"),
        other => panic!("expected Decimal, got {:?}", other),
    }
    match parse_ok("1_000.25M") {
        Literal::Decimal(s) => assert_eq!(s, "1000.25"),
        other => panic!("expected Decimal, got {:?}", other),
    }
}
