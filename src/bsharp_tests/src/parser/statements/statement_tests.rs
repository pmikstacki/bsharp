...
use parser::statement_parser::{debug_test_individual_parsers, parse_statement};
use parser::syntax::test_helpers::parse_statement_all;
use syntax::nodes::expressions::expression::Expression;
use syntax::nodes::expressions::literal::Literal;
use syntax::nodes::identifier::Identifier;
use syntax::nodes::statements::statement::{Statement, Statement::*};
...
fn test_parse_expression_statement() {
    let code = "foo;";
    let expected = Expression(Expression::Variable(Identifier {
        name: "foo".to_string(),
    }));
...
fn test_parse_statement_dispatch() {
    assert!(parse_statement_all("if (true) {} ").is_ok());
    assert!(parse_statement_all("while (false) break;").is_ok());
    assert!(parse_statement_all("MyFunc();").is_ok());
    assert!(parse_statement_all("{} ").is_ok());
    assert!(parse_statement_all("int y;").is_ok());
}
#[test]
...
fn test_direct_block_statement_call() {
    let input = "{ foo; 42; }";
    println!(
        "TESTING DIRECT parse_block_statement call with input: '{}'",
        input
    );
    ...
    match parse_block_statement(input) {
        Ok((remaining, stmt)) => {
            println!(
                "✅ DIRECT parse_block_statement SUCCESS - remaining: {:?}, stmt: {:?}",
                remaining, stmt
            );
        }
        Err(e) => {
