...
fn test_parse_for_statement_logic() {
    ...
    match stmt_simple {
        Statement::For(boxed_for_stmt) => {
            ...
            assert!(matches!(
                ...
                Some(ForInitializer::Declaration(_))
            ));
    ...
    let result_multiple_init = parse_for_statement(input_multiple_init.into());
    assert!(result_multiple_init.is_ok());
    let (remaining_multiple_init, stmt_multiple_init) = result_multiple_init.unwrap();
    assert!(
        remaining_multiple_init.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_multiple_init.fragment()
    );
    match stmt_multiple_init {
        Statement::For(boxed_for_stmt) => {
            let for_stmt = &*boxed_for_stmt;
            match &for_stmt.initializer {
                Some(ForInitializer::Declaration(LocalVariableDeclaration {
                    declarators, ..
                })) => {
    ...
    let result_multiple_iter = parse_for_statement(input_multiple_iter.into());
    assert!(result_multiple_iter.is_ok());
    let (remaining_multiple_iter, stmt_multiple_iter) = result_multiple_iter.unwrap();
    assert!(
        remaining_multiple_iter.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_multiple_iter.fragment()
    );
    ...
    let result_no_init = parse_for_statement(input_no_init.into());
    assert!(result_no_init.is_ok());
    let (remaining_no_init, stmt_no_init) = result_no_init.unwrap();
    assert!(
        remaining_no_init.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_no_init.fragment()
    );
    match stmt_no_init {
        Statement::For(boxed_for_stmt) => {
            let for_stmt = &*boxed_for_stmt;
    ...
    let result_no_cond = parse_for_statement(input_no_cond.into());
    assert!(result_no_cond.is_ok());
    let (remaining_no_cond, stmt_no_cond) = result_no_cond.unwrap();
    assert!(
        remaining_no_cond.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_no_cond.fragment()
    );
    match stmt_no_cond {
        Statement::For(boxed_for_stmt) => {
            let for_stmt = &*boxed_for_stmt;
    ...
    let result_no_iter = parse_for_statement(input_no_iter.into());
    assert!(result_no_iter.is_ok());
    let (remaining_no_iter, stmt_no_iter) = result_no_iter.unwrap();
    assert!(
        remaining_no_iter.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_no_iter.fragment()
    );
    match stmt_no_iter {
        Statement::For(boxed_for_stmt) => {
            let for_stmt = &*boxed_for_stmt;
    ...
    let result_empty = parse_for_statement(input_empty.into());
    assert!(result_empty.is_ok());
    let (remaining_empty, stmt_empty) = result_empty.unwrap();
    assert!(
        remaining_empty.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_empty.fragment()
    );
    match stmt_empty {
        Statement::For(boxed_for_stmt) => {
            let for_stmt = &*boxed_for_stmt;
    ...
    let result_expr_init = parse_for_statement(input_expr_init.into());
    assert!(result_expr_init.is_ok());
    let (remaining_expr_init, stmt_expr_init) = result_expr_init.unwrap();
    assert!(
        remaining_expr_init.fragment().trim().is_empty(),
        "Input not fully consumed: {}",
        remaining_expr_init.fragment()
    );
