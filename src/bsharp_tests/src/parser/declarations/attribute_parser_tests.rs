...
fn test_single_attribute_no_args() {
    ...
    assert_eq!(lists[0].attributes[0].name.to_string(), "Serializable");
...
fn test_multiple_attribute_lists() {
    ...
    let (rest, lists) = parse_attribute_lists(input.into()).unwrap();
    assert_eq!(rest.fragment().to_string(), "class");
    assert_eq!(lists.len(), 2);
    assert_eq!(lists[0].attributes[0].name.to_string(), "Serializable");
    assert_eq!(lists[1].attributes[0].name.to_string(), "DataContract");
}
...
fn test_attribute_with_argument() {
    ...
    assert_eq!(lists.len(), 1);
    assert_eq!(lists[0].attributes[0].name.to_string(), "DataMember");
...
fn test_attribute_with_named_arguments() {
    ...
    let (rest, lists) =
        parse_attribute_lists(input.into()).expect("should parse attribute with named arg");
    assert_eq!(rest.fragment().to_string(), "".to_string());
    assert_eq!(lists.len(), 1);
    assert_eq!(lists[0].attributes.len(), 1);
...
fn test_multiple_attributes_in_one_list() {
    ...
    assert_eq!(lists[0].attributes[0].name.to_string(), "Serializable");
    assert_eq!(lists[0].attributes[1].name.to_string(), "DataContract");
}
...
fn test_attribute_with_multiple_arguments() {
    ...
    let (rest, lists) =
        parse_attribute_lists(input.into()).expect("should parse attribute with mixed args");
