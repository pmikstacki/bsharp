...
fn test_simple_interface_declaration() {
    ...
    let result = parse_interface_declaration(input.into());
    assert!(result.is_ok());
    ...
    assert_eq!(actual.name.to_string(), "IMyInterface");
...
fn test_interface_with_modifiers() {
    ...
    assert_eq!(decl.name.to_string(), "IMyInterface");
}
...
fn test_interface_with_base_types() {
    ...
    if let Type::Generic { base, args: _ } = &decl.base_types[0] {
        assert_eq!(base.to_string(), "ICollection");
    } else {
...
fn test_interface_with_attributes() {
    ...
    let (remaining, decl) = result.unwrap();
    assert!(remaining.trim().is_empty());
    assert_eq!(decl.name.to_string(), "ISerializable");
...
fn test_interface_with_method_signature() {
    ...
    assert_eq!(decl.name.to_string(), "ITest");
    ...
    match &decl.body_declarations[0] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "DoSomething");
            assert!(
                method.body.is_none(),
...
fn test_interface_with_multiple_method_signatures() {
    ...
    assert_eq!(decl.name.to_string(), "IMethods");
    assert_eq!(decl.modifiers, vec![Modifier::Public]);
    ...
    match &decl.body_declarations[0] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "Method1");
    ...
    match &decl.body_declarations[1] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "Method2");
...
fn test_interface_method_with_body_error() {
    ...
    assert_eq!(decl.name.to_string(), "IInvalid");
    assert_eq!(
        decl.body_declarations.len(),
        1,
    ...
    match &decl.body_declarations[0] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "Problem");
        }
...
fn test_interface_with_generic_method() {
    ...
    let (remaining, decl) = result.unwrap();
    assert!(remaining.trim().is_empty());
    assert_eq!(decl.name.to_string(), "IGenericInterface");
    assert_eq!(decl.body_declarations.len(), 1);

    // Check the method is parsed correctly
    match &decl.body_declarations[0] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "Process");
            ...
            assert_eq!(method_tp[0].name.to_string(), "T");
            assert_eq!(method.parameters.len(), 1, "Expected one parameter");
            assert_eq!(method.parameters[0].name.to_string(), "input");
        }
        _ => panic!("Expected a method member"),
    }
}
...
fn test_complex_interface_with_multiple_methods() {
    ...
    let (remaining, decl) = result.unwrap();
    assert!(remaining.trim().is_empty());
    assert_eq!(decl.name.to_string(), "IComplex");
    assert!(decl.type_parameters.is_some());
    let tp = decl.type_parameters.as_ref().unwrap();
    assert_eq!(tp.len(), 1);
    assert_eq!(decl.modifiers, vec![Modifier::Public]);
    ...
    match &decl.body_declarations[0] {
        InterfaceBodyDeclaration::Method(method) => {
            ...
            assert_eq!(method.parameters[0].name.to_string(), "id");
            assert_eq!(method.parameters[1].name.to_string(), "name");
            assert_eq!(method.parameters[2].name.to_string(), "value");
        }
    ...
    match &decl.body_declarations[1] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "GetValue");
            assert!(method.parameters.is_empty(), "Expected no parameters");
        }
    ...
    match &decl.body_declarations[2] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "SetValue");
            assert_eq!(method.parameters.len(), 1, "Expected one parameter");
            assert_eq!(method.parameters[0].name.to_string(), "value");
        }
    ...
    match &decl.body_declarations[3] {
        InterfaceBodyDeclaration::Method(method) => {
            assert_eq!(method.name.to_string(), "Convert");
            assert!(method.type_parameters.is_some());
            let method_tp = method.type_parameters.as_ref().unwrap();
            assert_eq!(
                method_tp.len(),
...
fn test_interface_with_irregular_whitespace() {
    ...
    assert_eq!(decl.name.to_string(), "IMalformatted");
...
fn test_error_recovery_with_malformed_member() {
    ...
    assert_eq!(decl.name.to_string(), "IErrorRecovery");
