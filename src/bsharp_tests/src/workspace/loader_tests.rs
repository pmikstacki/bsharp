...
fn loader_from_csproj_single_project() {
    ...
    assert!(
        ws.projects[0]
            .files
            .iter()
            .any(|f| f.path.ends_with("Program.cs"))
    );
}
...
fn loader_from_sln_with_transitive_project_references() {
    ...
    write(
        &proj_a,
        r#"<Project Sdk="Microsoft.NET.Sdk"><ItemGroup><ProjectReference Include="../B/B.csproj" /></ItemGroup></Project>"#,
    );
    ...
    write(
        &proj_b,
        r#"<Project Sdk="Microsoft.NET.Sdk"><ItemGroup><ProjectReference Include="../C/C.csproj" /></ItemGroup></Project>"#,
    );
