...
impl CsprojReader {
    pub fn read(path: &Path) -> Result<Project> {
            ...
            match reader.read_event_into(&mut buf) {
                Ok(Event::Empty(e)) | Ok(Event::Start(e)) => {
                    ...
                    for a in e.attributes().flatten() {
                        ...
                        let raw = a.unescape_value().ok().map(|v| v.to_string()).unwrap_or_default();
                        // Tests may use backslash-escaped quotes in raw strings; normalize those.
                        let dequoted = raw.replace("\\\"", "\"").replace("\\'", "'");
                        // Trim whitespace and surrounding quotes
                        let trimmed = dequoted.trim().trim_matches(|c| c == '"' || c == '\'').to_string();
                        ...
                        if name.ends_with(b"compile") {
                            if key.eq_ignore_ascii_case(b"include") || key.eq_ignore_ascii_case(b"update") {
                            ...
                            if key.eq_ignore_ascii_case(b"remove") {
                                if !val_clean.is_empty() {
                                    if val_clean.contains("$(") {
                                        project.errors.push(format!(
                        ...
                        if name.ends_with(b"projectreference") && key.eq_ignore_ascii_case(b"include") {
                            if val_clean.contains("$(") {
                                project.errors.push(format!(
                            ...
                            match p.canonicalize() {
                                ...
                                Err(_) => {
                                    ...
                                    project
                                        .errors
                                        .push(format!("Unresolved ProjectReference: {}", p.display()));
                                }
                            }
                        }
                    }
        ...
        for f in file_set {
            if f.extension().and_then(|s| s.to_str()).map(|e| e.eq_ignore_ascii_case("cs")).unwrap_or(false) {
                project.add_file(ProjectFile::new_source(f, Language::CSharp));
            } else {
                project.add_file(ProjectFile { path: f, kind: ProjectFileKind::Other, language: None });
            }
        }
    ...
    fn remove_glob(root: &Path, pattern: &str, files: &mut Vec<PathBuf>) {
        if let Ok(w) = GlobWalkerBuilder::from_patterns(root, &[pattern]).case_insensitive(true).build() {
        ...
        let is_simple = !pattern.contains('/') && !pattern.contains('\\') && !pattern.contains('*') && !pattern.contains('?') && !pattern.contains('[');
        if is_simple {
            files.retain(|p| p.file_name().and_then(|n| n.to_str()).map(|n| n != pattern).unwrap_or(true));
        } else {
