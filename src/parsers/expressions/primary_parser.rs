...
fn parse_variable_reference(input: &str) -> IResult<&str, Expression> {
...
fn parse_literal_expression(input: &str) -> IResult<&str, Expression> {
    map(parse_literal, Expression::Literal)(input)
}

// Parse a primary expression (variable, literal, parenthesized)
pub fn parse_primary_expression(input: &str) -> IResult<&str, Expression> {
    alt((
