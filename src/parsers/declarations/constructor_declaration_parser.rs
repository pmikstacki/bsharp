...
mod tests {
    use super::*;
    use crate::parser::nodes::statements::statement::Statement; // Ensure this import is present
    ...
    fn test_constructor_with_public_modifier() {
        ...
        match parse_constructor_declaration(input) {
            Ok((remaining, constructor_decl)) => {
                ...
                assert!(constructor_decl.modifiers.contains(&crate::parser::nodes::declarations::modifier::Modifier::Public));
            }
            Err(e) => panic!("Parser failed with error: {:?} for input: {}", e, input),
        }
    }
