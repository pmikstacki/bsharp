...
use bsharp_syntax::declarations::{
    ...
    InterfaceBodyDeclaration, InterfaceDeclaration, NamespaceDeclaration, RecordDeclaration,
    StructBodyDeclaration, StructDeclaration,
};
...
impl TypeAnalyzer {
        ...
        for decl in &unit.declarations {
            if let TopLevelDeclaration::Class(c) = decl {
                ...
                for m in &c.body_declarations {
                    use ClassBodyDeclaration as CBD;
                    match m {
                        CBD::Field(f) => record_type_to_usage(&mut usage, &f.field_type),
                        CBD::Property(p) => record_type_to_usage(&mut usage, &p.property_type),
                        CBD::Method(md) => {
                            record_type_to_usage(&mut usage, &md.return_type);
                            for par in &md.parameters {
                                record_type_to_usage(&mut usage, &par.parameter_type);
                            }
                        }
                        _ => {}
                    }
                    ...
                    match m {
                        CBD::Field(f) => {
                            if let Type::Primitive(p) = &f.field_type {
                                set.insert(format!("{:?}", p).to_lowercase());
                            }
                        }
                        CBD::Property(p) => {
                            if let Type::Primitive(pr) = &p.property_type {
                                set.insert(format!("{:?}", pr).to_lowercase());
                            }
                        }
                        CBD::Method(md) => {
                            if let Type::Primitive(pr) = &md.return_type {
                                set.insert(format!("{:?}", pr).to_lowercase());
                            }
                            for par in &md.parameters {
                                if let Type::Primitive(pr) = &par.parameter_type {
                                    set.insert(format!("{:?}", pr).to_lowercase());
                                }
                            }
                        }
    ...
    ) -> std::collections::HashSet<PrimitiveType> {
        use PrimitiveType;
        ...
        for decl in &unit.declarations {
            if let TopLevelDeclaration::Class(c) = decl {
                ...
                for m in &c.body_declarations {
                    ...
                    match m {
                        CBD::Field(f) => {
                            if let Type::Reference(id) = &f.field_type {
                                set.insert(id.name.clone());
                            }
                        }
                        CBD::Property(p) => {
                            if let Type::Reference(id) = &p.property_type {
                                set.insert(id.name.clone());
                            }
                        }
                        CBD::Method(md) => {
                            if let Type::Reference(id) = &md.return_type {
                                set.insert(id.name.clone());
                            }
                            for par in &md.parameters {
                                if let Type::Reference(id) = &par.parameter_type {
                                    set.insert(id.name.clone());
                                }
                            }
                        }
    ...
    pub fn analyze_type_complexity(&self, type_ref: &Type) -> TypeComplexity {
        match type_ref {
            ...
            Type::Generic { args, .. } => {
                let nested_generics = args
                    .iter()
                    .filter(|a| matches!(a, Type::Generic { .. }))
                    .count();
                if nested_generics > 0 || args.len() > 2 {
                    TypeComplexity::VeryComplex
                } else {
    ...
    pub fn detect_circular_dependencies(&self) -> Vec<Vec<String>> {
        ...
        fn visit(
            ...
            if let Some(nexts) = rel.get(node) {
                for n in nexts {
                    visit(n, rel, temp, perm, stack, out);
                }
            }
            stack.pop();
            temp.remove(node);
    ...
    fn analyze_class_member(&self, member: &ClassBodyDeclaration, metrics: &mut TypeMetrics) {
        use ClassBodyDeclaration;

        match member {
            ClassBodyDeclaration::Field(field) => {
                let type_name = Self::extract_type_name(&field.field_type);
                metrics.field_types.push(type_name);
                Self::analyze_type_for_metrics(&field.field_type, metrics);
            }
            ClassBodyDeclaration::Property(property) => {
                let type_name = Self::extract_type_name(&property.property_type);
                metrics.property_types.push(type_name);
                Self::analyze_type_for_metrics(&property.property_type, metrics);
            }
            ClassBodyDeclaration::Method(method) => {
                ...
                for param in &method.parameters {
                    ...
                    if let Some(modifier) = &param.modifier {
                        match modifier {
                            ParameterModifier::Ref => metrics.ref_parameters += 1,
                            ParameterModifier::Out => metrics.out_parameters += 1,
                            _ => {}
                        }
                    }
                }
    ...
    fn count_class_members(members: &Vec<ClassBodyDeclaration>) -> MemberCounts {
        use ClassBodyDeclaration as CBD;
        ...
        for m in members {
            match m {
                ...
                | CBD::NestedClass(_)
                | CBD::NestedStruct(_)
                | CBD::NestedInterface(_)
                | CBD::NestedEnum(_)
                | CBD::NestedRecord(_) => mc.nested_types += 1,
            }
        }
        mc
    }
    ...
    fn count_struct_members(members: &Vec<StructBodyDeclaration>) -> MemberCounts {
        use StructBodyDeclaration as SBD;
        let mut mc = MemberCounts::default();
        for m in members {
            match m {
                ...
                SBD::NestedClass(_)
                | SBD::NestedStruct(_)
                | SBD::NestedInterface(_)
                | SBD::NestedEnum(_)
                | SBD::NestedRecord(_) => mc.nested_types += 1,
            }
        }
        mc
    }
    ...
    fn count_interface_members(members: &Vec<InterfaceBodyDeclaration>) -> MemberCounts {
        use InterfaceBodyDeclaration as IBD;
        let mut mc = MemberCounts::default();
        for m in members {
            match m {
                ...
                IBD::NestedClass(_)
                | IBD::NestedStruct(_)
                | IBD::NestedInterface(_)
                | IBD::NestedEnum(_)
                | IBD::NestedRecord(_) => mc.nested_types += 1,
            }
        }
        mc
    }
    ...
    fn walk_namespace(
        ...
        for member in &ns.declarations {
            use bsharp_syntax::nodes::declarations::NamespaceBodyDeclaration as NBD;
            match member {
    ...
    fn record_class(&mut self, c: &ClassDeclaration, ns: &[&str], enclosing: &[&str]) {
        ...
        for bt in &c.base_types {
            if let Type::Reference(id) = bt {
                bases.push(id.name.clone());
                if !ns.is_empty() {
                    bases.push(format!("{}.{}", ns.join("."), id.name));
                }
            }
        }
    ...
    fn record_struct(&mut self, s: &StructDeclaration, ns: &[&str], enclosing: &[&str]) {
        let mut bases: Vec<String> = Vec::new();
        ...
        use StructBodyDeclaration as SBD;
        let mut new_enclosing: Vec<&str> = enclosing.to_vec();
        new_enclosing.push(&s.name.name);
    ...
    fn record_interface(&mut self, i: &InterfaceDeclaration, ns: &[&str], enclosing: &[&str]) {
        let mut bases: Vec<String> = Vec::new();
        ...
        use InterfaceBodyDeclaration as IBD;
        let mut new_enclosing: Vec<&str> = enclosing.to_vec();
        new_enclosing.push(&i.name.name);
    ...
    fn record_enum(&mut self, e: &EnumDeclaration, ns: &[&str], enclosing: &[&str]) {
        ...
        self.discovered_types.insert(
            ...
            TypeInfo {
                ...
                kind: TypeKind::Enum,
                base_types: Vec::new(),
                member_counts: MemberCounts::default(),
            },
        );
    }

    fn record_record(&mut self, r: &RecordDeclaration, ns: &[&str], enclosing: &[&str]) {
        let mut bases: Vec<String> = Vec::new();
    ...
    fn record_delegate(&mut self, d: &DelegateDeclaration, ns: &[&str], enclosing: &[&str]) {
