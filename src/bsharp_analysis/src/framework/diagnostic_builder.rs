...
pub struct DiagnosticBuilder {
    ...
    location: Option<crate::diagnostics::source_location::SourceLocation>,
    related: Vec<crate::diagnostics::diagnostic_related_information::DiagnosticRelatedInformation>,
}
...
impl DiagnosticBuilder {
    ...
    pub fn with_related_at_key(
        ...
        if let Some(range) = session.spans.get(key) {
            let info = crate::diagnostics::diagnostic_related_information::DiagnosticRelatedInformation::from_span(&session.ctx, range.start, range.end - range.start, message);
