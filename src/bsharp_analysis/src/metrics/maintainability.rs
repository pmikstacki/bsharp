use super::basic::{BasicMetrics, BasicMetricsCollector};
...
impl MaintainabilityAnalyzer {
    ...
    pub fn analyze_compilation_unit(&self, unit: &CompilationUnit) -> MaintainabilityMetrics {
        ...
        let mut basic_collector = BasicMetricsCollector::new();
        ...
        let mut complexity_total = ComplexityMetrics::default();
        ...
        for decl in &unit.declarations {
            if let crate::syntax::ast::TopLevelDeclaration::Class(class) = decl {
                for m in &class.body_declarations {
                    if let crate::syntax::nodes::declarations::ClassBodyDeclaration::Method(
                        method,
                    ) = m
                    {
                        ...
                        complexity_total.max_nesting_depth =
                            complexity_total.max_nesting_depth.max(cm.max_nesting_depth);
                        complexity_total.abc_complexity.assignments +=
                            cm.abc_complexity.assignments;
