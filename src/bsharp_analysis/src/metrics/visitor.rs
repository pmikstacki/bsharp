...
impl Visit for MetricsVisitor {
                        ...
                        match m {
                            ClassBodyDeclaration::Method(m) => {
                                ...
                                if let Some(body) = &m.body {
                                    count_statements(body, &mut self.analysis);
                                }
                            }
                            ClassBodyDeclaration::Constructor(_) => {
                                self.analysis.total_constructors += 1
                            }
                            ClassBodyDeclaration::Property(_) => {
                                self.analysis.total_properties += 1
                            }
    ...
    fn exit(&mut self, node: &NodeRef, session: &mut AnalysisSession) {
        if let NodeRef::CompilationUnit(_) = node {
    ...
    match stmt {
        ...
        Try(try_stmt) => {
            ...
            for h in &try_stmt.catches {
                count_statements(&h.block, analysis);
            }
            if let Some(fin) = &try_stmt.finally_clause {
                count_statements(&fin.block, analysis);
            }
        }
        Using(u) => {
            analysis.total_using_statements += 1;
            if let Some(body) = u.body.as_deref() {
                count_statements(body, analysis);
            }
        }
        Block(stmts) => {
            for s in stmts {
