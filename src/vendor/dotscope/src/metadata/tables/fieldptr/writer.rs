//! `FieldPtr` table binary writer implementation
//!
//! Provides binary serialization implementation for the `FieldPtr` metadata table (0x03) through
//! the [`crate::metadata::tables::types::RowWritable`] trait. This module handles the low-level
//! serialization of `FieldPtr` table entries to the metadata tables stream format.
//!
...
//! # Row Layout
//!
//! `FieldPtr` table rows are serialized with this binary structure:
//! - `field` (2/4 bytes): Field table index for indirection
//!
//! # Architecture
//!
...
//! - [`crate::metadata::tables::fieldptr::FieldPtrRaw`]: Raw field pointer data structure
...
impl RowWritable for FieldPtrRaw {
    /// Write a `FieldPtr` table row to binary data
    ///
    /// Serializes one `FieldPtr` table entry to the metadata tables stream format, handling
...
mod tests {
    ...
    fn test_round_trip_serialization_short() {
        ...
        let original_row = FieldPtrRaw {
            rid: 1,
            token: Token::new(0x03000001),
            offset: 0,
            field: 0x0101,
        };
        ...
        let row_size = <FieldPtrRaw as TableRow>::row_size(&table_info) as usize;
        ...
        let deserialized_row = FieldPtrRaw::row_read(&buffer, &mut read_offset, 1, &table_info)
            .expect("Deserialization should succeed");

        // Compare all fields
        assert_eq!(original_row.field, deserialized_row.field);
