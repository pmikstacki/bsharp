...
fn loads_fixture_solution_projects_and_files() {
    ...
    // Assert projects
    assert_eq!(ws.projects.len(), 2, "expected two projects in fixture (app + dependency)");

    // Assert presence of key source files
    let all_sources: Vec<_> = ws.all_source_files().iter().map(|p| p.to_path_buf()).collect();
    let has_program = all_sources.iter().any(|p| p.file_name().unwrap().to_string_lossy() == "Program.cs");
    let has_dep = all_sources.iter().any(|p| p.file_name().unwrap().to_string_lossy() == "TestDependency.cs");
