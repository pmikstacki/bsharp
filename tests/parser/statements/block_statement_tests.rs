...
fn test_parse_block_statement() {
    ...
    assert!(result_empty.is_ok(), "Parsing empty block failed: {:?}", result_empty.err());
    ...
    assert!(result_simple.is_ok(), "Parsing simple block failed: {:?}", result_simple.err());
    match result_simple.unwrap().1 {
        Statement::Block(stmts) => {
            ...
            assert!(matches!(stmts[0], Statement::Declaration(_)), "First statement was not a Declaration");
            assert!(matches!(stmts[1], Statement::Return(_)), "Second statement was not a Return");
        }
    ...
    assert!(result_nested.is_ok(), "Parsing nested block failed: {:?}", result_nested.err());
    match result_nested.unwrap().1 {
        Statement::Block(stmts) => {
            assert_eq!(stmts.len(), 1, "Outer block did not contain 1 statement");
            match &stmts[0] {
                Statement::Block(inner_stmts) => {
                    assert_eq!(inner_stmts.len(), 1, "Inner block did not contain 1 statement");
                    // Assuming "x = 5;" parses as an ExpressionStatement
                    assert!(matches!(inner_stmts[0], Statement::Expression(_)), "Innermost statement was not an Expression");
                }
...
fn test_parse_block_statement_empty() {
    ...
    assert!(result.is_ok(), "Parsing empty block failed: {:?}", result.err());
...
fn test_parse_block_statement_with_statements() {
    ...
    let result = all_consuming(parse_block_statement)(input).finish();
    assert!(result.is_ok(), "Parsing block with statements failed: {:?}", result.err());
        ...
        Statement::Block(statements) => {
            ...
            match &statements[0] {
                Statement::Declaration(LocalVariableDeclaration { declarators, declaration_type: ty, .. }) => {
                    ...
                    assert!(matches!(declarator.initializer, Some(Expression::Literal(Literal::Integer(1)))));
            ...
            match &statements[1] {
                Statement::Declaration(LocalVariableDeclaration { declarators, declaration_type: ty, .. }) => {
                    ...
                    assert!(matches!(&declarator.initializer, Some(Expression::Literal(Literal::String(s))) if s == "test"));
