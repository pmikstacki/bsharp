...
fn test_parse_simple_query_expression() {
    ...
    assert!(result.is_ok(), "Failed to parse simple query expression: {:?}", result);
...
fn test_parse_query_with_type_annotation() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with type annotation: {:?}", result);
...
fn test_parse_query_with_where_clause() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with where clause: {:?}", result);
...
fn test_parse_query_with_let_clause() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with let clause: {:?}", result);
...
fn test_parse_query_with_orderby_clause() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with orderby clause: {:?}", result);
    ...
    if let Ok(Expression::Query(query)) = result {
        ...
        if let QueryClause::OrderBy(orderby_clause) = &query.body[0] {
            ...
            assert!(ordering.direction.is_none() || matches!(ordering.direction, Some(OrderingDirection::Ascending)));
        } else {
...
fn test_parse_query_with_orderby_descending() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with orderby descending: {:?}", result);
    ...
    if let Ok(Expression::Query(query)) = result {
        ...
        if let QueryClause::OrderBy(orderby_clause) = &query.body[0] {
            let ordering = &orderby_clause.orderings[0];
            assert!(matches!(ordering.direction, Some(OrderingDirection::Descending)));
        } else {
            panic!("Expected orderby clause in body");
        }
    } else {
...
fn test_parse_query_with_group_clause() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with group clause: {:?}", result);
...
fn test_parse_complex_query_expression() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse complex query expression: {:?}", result);
...
fn test_parse_query_with_join_clause() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with join clause: {:?}", result);
...
fn test_parse_query_with_multiple_from_clauses() {
    ...
    let result = parse_query_test(code);
    assert!(result.is_ok(), "Failed to parse query with multiple from clauses: {:?}", result);
