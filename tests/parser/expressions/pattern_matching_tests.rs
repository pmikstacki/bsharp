...
use bsharp::parser::expressions::switch_expression_parser::{
...
fn test_parse_discard_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse discard pattern: {:?}",
        result
    );
...
fn test_parse_constant_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse constant pattern: {:?}",
        result
    );
...
fn test_parse_relational_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse relational pattern: {:?}",
        result
    );
...
fn test_parse_logical_and_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse logical AND pattern: {:?}",
        result
    );
...
fn test_parse_logical_or_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse logical OR pattern: {:?}",
        result
    );
...
fn test_parse_property_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse property pattern: {:?}",
        result
    );

    if let Ok(Pattern::Property {
        type_name,
        subpatterns,
    }) = result
    {
...
fn test_parse_positional_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse positional pattern: {:?}",
        result
    );

    if let Ok(Pattern::Positional {
        type_name,
        subpatterns,
    }) = result
    {
        ...
        assert!(
            subpatterns
                .iter()
                .all(|p| matches!(p, Pattern::Constant(_)))
        );
    } else {
...
fn test_parse_complex_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse complex pattern: {:?}",
        result
    );
...
fn test_parse_simple_switch_expression() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse simple switch expression: {:?}",
        result
    );
...
fn test_parse_switch_expression_with_when_clause() {
    let result =
        parse_switch_test("value switch { var x when x > 0 => \"positive\", _ => \"other\" }");
    assert!(
        result.is_ok(),
        "Failed to parse switch expression with when clause: {:?}",
        result
    );
    ...
    if let Ok(Expression::SwitchExpression(switch_expr)) = result {
        ...
        panic!(
            "Expected switch expression with when clause, got: {:?}",
            result
        );
    }
}
...
fn test_parse_switch_expression_with_patterns() {
    let result = parse_switch_test(
        "shape switch { Circle(var r) => r, Rectangle(var w, var h) => w * h, _ => 0 }",
    );
    assert!(
        result.is_ok(),
        "Failed to parse switch expression with patterns: {:?}",
        result
    );
    ...
    if let Ok(Expression::SwitchExpression(switch_expr)) = result {
        ...
        assert!(matches!(
            switch_expr.arms[0].pattern,
            Pattern::Positional { .. }
        ));
        assert!(matches!(
            switch_expr.arms[1].pattern,
            Pattern::Positional { .. }
        ));
        assert!(matches!(switch_expr.arms[2].pattern, Pattern::Discard));
    } else {
        panic!(
            "Expected switch expression with patterns, got: {:?}",
            result
        );
    }
}
...
fn test_parse_is_pattern_expression() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse is pattern expression: {:?}",
        result
    );

    if let Ok(Expression::IsPattern {
        expression,
        pattern,
    }) = result
    {
...
fn test_parse_is_pattern_with_property_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse is pattern with property pattern: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected is pattern with property pattern, got: {:?}",
            result
        );
    }
}
...
fn test_parse_list_pattern_with_slice() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse list pattern with slice: {:?}",
        result
    );
    ...
    if let Ok(Pattern::List { patterns }) = result {
        ...
        assert!(matches!(
            patterns[0],
            ListPatternElement::Pattern(Pattern::Var(_))
        ));
...
fn test_parse_record_pattern_property_syntax() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with property syntax: {:?}",
        result
    );
...
fn test_parse_record_pattern_multiple_properties() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with multiple properties: {:?}",
        result
    );
...
fn test_parse_record_pattern_with_var_patterns() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with var patterns: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected record property pattern with var patterns, got: {:?}",
            result
        );
    }
}
...
fn test_parse_record_pattern_positional_syntax() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with positional syntax: {:?}",
        result
    );
...
fn test_parse_record_pattern_positional_with_var() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with positional var: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected record positional pattern with var, got: {:?}",
            result
        );
    }
}
...
fn test_parse_record_pattern_positional_mixed() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with mixed positional: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected record positional pattern with mixed patterns, got: {:?}",
            result
        );
    }
}
...
fn test_parse_nested_record_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse nested record pattern: {:?}",
        result
    );
...
fn test_parse_record_pattern_with_discard() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with discard: {:?}",
        result
    );
...
fn test_parse_generic_record_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse generic record pattern: {:?}",
        result
    );
    ...
    }) = result
    {
        assert!(
            type_name.is_some(),
            "Expected type name for generic record pattern"
        );
        assert_eq!(subpatterns.len(), 1);
...
fn test_parse_qualified_record_pattern() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse qualified record pattern: {:?}",
        result
    );
...
fn test_parse_record_pattern_in_switch_expression() {
    let result = parse_switch_test(
        "person switch { Person { Age: > 18 } => \"adult\", Person { Age: <= 18 } => \"minor\", _ => \"unknown\" }",
    );
    assert!(
        result.is_ok(),
        "Failed to parse switch expression with record patterns: {:?}",
        result
    );
    ...
    if let Ok(Expression::SwitchExpression(switch_expr)) = result {
        ...
        panic!(
            "Expected switch expression with record patterns, got: {:?}",
            result
        );
    }
}
...
fn test_parse_record_pattern_with_relational_and_logical() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse record pattern with complex sub-patterns: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected record pattern with complex sub-patterns, got: {:?}",
            result
        );
    }
}
...
fn test_parse_record_pattern_empty() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse empty record pattern: {:?}",
        result
    );
...
fn test_parse_positional_record_pattern_empty() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse empty positional record pattern: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected empty positional record pattern, got: {:?}",
            result
        );
    }
}
...
fn test_parse_complex_nested_record_pattern() {
    let result = parse_pattern_test(
        "Order { Customer: Person { Name: var customerName }, Items: [var first, ..] }",
    );
    assert!(
        result.is_ok(),
        "Failed to parse complex nested record pattern: {:?}",
        result
    );
...
fn test_parse_record_pattern_is_expression() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse is expression with record pattern: {:?}",
        result
    );
    ...
    }) = result
    {
        ...
        panic!(
            "Expected is expression with record pattern, got: {:?}",
            result
        );
    }
}
