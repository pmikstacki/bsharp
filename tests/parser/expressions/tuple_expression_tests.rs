...
fn check_tuple_expr(input: &str, expected_elements: Vec<TupleElement>) {
    ...
    let expected_expr = Expression::Tuple(TupleExpression { elements: expected_elements });
...
fn test_simple_tuple_expression() {
    check_tuple_expr(
        "(1, 2)",
        vec![
            TupleElement { name: None, value: Expression::Literal(Literal::Integer(1)) },
            TupleElement { name: None, value: Expression::Literal(Literal::Integer(2)) },
        ],
    );
}
...
fn test_tuple_with_more_elements() {
    check_tuple_expr(
        r#"("a", true, 3.0)"#,
        vec![
            TupleElement { name: None, value: Expression::Literal(Literal::String("a".to_string())) },
            TupleElement { name: None, value: Expression::Literal(Literal::Boolean(true)) },
            TupleElement { name: None, value: Expression::Literal(Literal::Float(3.0)) },
        ],
    );
}
...
fn test_tuple_with_mixed_named_and_unnamed_elements() {
    check_tuple_expr(
        ...
        vec![
            ...
            TupleElement { name: None, value: Expression::Literal(Literal::Integer(3)) },
        ],
    );
}
            ...
            TupleElement {
                ...
                value: Expression::Tuple(TupleExpression {
                    elements: vec![
                        ...
                        TupleElement { name: None, value: Expression::Literal(Literal::Integer(4)) },
                    ],
                }),
            },
        ],
...
fn test_single_element_not_a_tuple() {
    ...
    assert!(result.is_ok(), "Expected parenthesized expression, not tuple error. Input: (1)");
    let expr = result.unwrap().1;
    assert_eq!(expr, Expression::Literal(Literal::Integer(1)), "Expected parenthesized literal 1. Input: (1)");

    let result_named = parse_expression("(a: 1)");
     assert!(result_named.is_err(), "Expected error for named single element tuple. Input: (a:1)");
}
...
fn test_tuple_with_just_comma_is_error() {
    let result = parse_expression("(,)");
    assert!(result.is_err(), "Expected error for tuple with just comma. Input: (,)");
}

#[test]
fn test_tuple_missing_closing_paren_is_error() {
    let result = parse_expression("(1, 2");
    assert!(result.is_err(), "Expected error for missing closing paren. Input: (1, 2");
}

#[test]
fn test_tuple_missing_comma_is_error() {
    let result = parse_expression("(1 2)");
     assert!(result.is_err(), "Expected error for missing comma. Input: (1 2)");
}
...
fn test_tuple_invalid_element_name() {
    ...
    assert!(result.is_err(), "Expected error for invalid element name. Input: (1: x, 2)");
}
