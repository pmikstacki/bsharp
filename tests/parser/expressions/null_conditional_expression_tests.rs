...
fn test_parse_null_conditional_indexing() {
    let code = "arr?[0]";
    let result = parse_null_conditional_expr_helper(code);
    assert!(result.is_ok(), "Failed to parse null-conditional indexing: {:?}", result);
...
fn test_parse_chained_null_conditional() {
    ...
    let result = parse_null_conditional_expr_helper(code);
    assert!(result.is_ok(), "Failed to parse chained null-conditional: {:?}", result);
    
    if let Ok(Expression::NullConditional(outer_null_cond)) = result {
        assert!(!outer_null_cond.is_element_access);
        assert_eq!(outer_null_cond.member.name, "Property");
        ...
        if let Expression::NullConditional(inner_null_cond) = &*outer_null_cond.target {
            ...
            assert_eq!(inner_null_cond.member.name, "Child");
...
fn test_parse_null_conditional_with_method_call() {
    let code = "obj?.Method()";
    let result = parse_null_conditional_expr_helper(code);
    assert!(result.is_ok(), "Failed to parse null-conditional with method call: {:?}", result);
    ...
    if let Ok(Expression::Invocation(invocation)) = result {
        ...
        if let Expression::NullConditional(null_cond) = &*invocation.callee {
            assert!(!null_cond.is_element_access);
            assert_eq!(null_cond.member.name, "Method");
...
fn test_parse_null_conditional_indexing_with_expression() {
    let code = "dict?[key + 1]";
    let result = parse_null_conditional_expr_helper(code);
    assert!(result.is_ok(), "Failed to parse null-conditional indexing with expression: {:?}", result);
...
fn test_parse_null_conditional_vs_ternary() {
    ...
    let result = parse_null_conditional_expr_helper(code);
    assert!(result.is_ok(), "Failed to parse ternary with null-conditional syntax: {:?}", result);
...
fn test_parse_null_conditional_whitespace_variations() {
    ...
    for code in variations {
        ...
        if code == "obj?.Property" {
            assert!(result.is_ok(), "Failed to parse standard null-conditional: {:?}", result);
        }
    }
}
