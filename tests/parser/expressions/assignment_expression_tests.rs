...
fn test_null_coalescing_assignment() {
    let code = "x ??= 42";
    let result = parse_assignment_expr_helper(code);
    assert!(result.is_ok(), "Parsing 'x ??= 42' failed: {:?}", result.err());
    let assignment_expr = result.unwrap();
    ...
    match *assignment_expr.value {
        ...
        _ => panic!("Expected value to be Expression::Literal(Integer(42)), got {:?}", assignment_expr.value),
    }
}
...
fn test_null_coalescing_assignment_with_complex_expression() {
    ...
    let result = parse_assignment_expr_helper(code);
    assert!(result.is_ok(), "Parsing 'data.Value ??= GetDefaultValue()' failed: {:?}", result.err());
    let assignment_expr = result.unwrap();
    ...
    match *assignment_expr.target {
        ...
        _ => panic!("Expected target to be Expression::MemberAccess, got {:?}", assignment_expr.target),
    }

    // Value should be an invocation expression
    match *assignment_expr.value {
        Expression::Invocation(_) => { /* Correct */ }
        _ => panic!("Expected value to be Expression::Invocation, got {:?}", assignment_expr.value),
    }
}
...
fn test_null_coalescing_assignment_chain() {
    let code = "a ??= b ??= c";
    let result = parse_assignment_expr_helper(code);
    assert!(result.is_ok(), "Parsing 'a ??= b ??= c' failed: {:?}", result.err());
    let assignment_expr = result.unwrap();
    ...
    match *assignment_expr.value {
        ...
        _ => panic!("Expected value to be another Assignment expression for right-associativity, got {:?}", assignment_expr.value),
    }
}
...
fn test_null_coalescing_vs_null_coalescing_assignment() {
    ...
    let code = "result = x ?? y";
    let result = parse_assignment_expr_helper(code);
    assert!(result.is_ok(), "Parsing 'result = x ?? y' failed: {:?}", result.err());
    let assignment_expr = result.unwrap();
    ...
    match *assignment_expr.value {
        Expression::Binary { op: BinaryOperator::NullCoalescing, .. } => { /* Correct */ }
        _ => panic!("Expected value to be Binary expression with NullCoalescing, got {:?}", assignment_expr.value),
    }
}
...
fn test_null_coalescing_assignment_precedence() {
    let code = "x = y ??= z + 1";
    let result = parse_assignment_expr_helper(code);
    assert!(result.is_ok(), "Parsing 'x = y ??= z + 1' failed: {:?}", result.err());
    let assignment_expr = result.unwrap();
    ...
    match *assignment_expr.value {
        Expression::Assignment(inner_assignment) => {
            ...
            match *inner_assignment.value {
                Expression::Binary { op: BinaryOperator::Add, .. } => { /* Correct */ }
                _ => panic!("Expected right side of ??= to be addition, got {:?}", inner_assignment.value),
            }
        }
        _ => panic!("Expected value to be Assignment expression, got {:?}", assignment_expr.value),
    }
}
