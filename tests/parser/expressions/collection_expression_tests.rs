...
use bsharp::syntax::nodes::expressions::expression::{CollectionElement, Expression};
...
fn test_basic_collection_expression() {
    ...
    match expr {
        Expression::Collection(elems) => {
            ...
            assert!(matches!(elems[0], CollectionElement::Expr(Expression::Literal(Literal::Integer(1)))));
        }
...
fn test_collection_with_spread_and_binary() {
    ...
    match expr {
        Expression::Collection(elems) => {
            ...
            // third is spread
            match &elems[2] {
                CollectionElement::Spread(Expression::Variable(Identifier { name })) => assert_eq!(name, "other"),
