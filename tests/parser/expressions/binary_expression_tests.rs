...
fn test_simple_binary_expression() {
    ...
    let input = "a * b";
    let expected = Expression::Binary {
        left: Box::new(Expression::Variable(Identifier { name: "a".to_string() })),
        op: BinaryOperator::Multiply,
        right: Box::new(Expression::Variable(Identifier { name: "b".to_string() })),
    };
    let (_, actual) = parse_expression(input).unwrap();
    assert_eq!(actual, expected);
}
...
fn test_compound_assignment_expression() {
    let input = "y += 5";
    let expected = Expression::Assignment(Box::new(AssignmentExpression {
        target: Box::new(Expression::Variable(Identifier { name: "y".to_string() })),
    ...
    let input = "z *= a";
    let expected = Expression::Assignment(Box::new(AssignmentExpression {
        target: Box::new(Expression::Variable(Identifier { name: "z".to_string() })),
        op: BinaryOperator::MultiplyAssign,
        value: Box::new(Expression::Variable(Identifier { name: "a".to_string() })),
    }));
    let (_, actual) = parse_expression(input).unwrap();
    assert_eq!(actual, expected);
}
...
fn test_precedence() {
    ...
    let expected = Expression::Binary {
        ...
        right: Box::new(Expression::Binary {
            left: Box::new(Expression::Variable(Identifier { name: "b".to_string() })),
            op: BinaryOperator::Multiply,
            right: Box::new(Expression::Variable(Identifier { name: "c".to_string() })),
        }),
    };
    let (_, actual) = parse_expression(input).unwrap();
    assert_eq!(actual, expected);
    ...
    let expected = Expression::Assignment(Box::new(AssignmentExpression {
        ...
        value: Box::new(Expression::Binary {
            left: Box::new(Expression::Variable(Identifier { name: "y".to_string() })),
            op: BinaryOperator::Equal,
            right: Box::new(Expression::Variable(Identifier { name: "z".to_string() })),
        }),
    }));
    let (_, actual) = parse_expression(input).unwrap();
    assert_eq!(actual, expected);
