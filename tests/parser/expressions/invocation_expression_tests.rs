...
fn parse_invocation_ok(code: &str) -> bsharp::syntax::nodes::expressions::invocation_expression::InvocationExpression {
...
fn invocation_with_modifiers() {
    ...
    assert!(matches!(inv.arguments[0].modifier, Some(ArgumentModifier::Ref)));
    assert!(matches!(inv.arguments[1].modifier, Some(ArgumentModifier::Out)));
    assert!(matches!(inv.arguments[2].modifier, Some(ArgumentModifier::In)));
}
...
fn invocation_with_named_arguments_and_calls() {
    ...
    if let Expression::Invocation(_) = inv.arguments[1].expr { /* ok */ } else { panic!("expected invocation in q:"); }
}
