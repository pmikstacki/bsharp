...
use bsharp::syntax::nodes::declarations::InterfaceBodyDeclaration;
...
"#;
    ...
    assert!(result.is_ok(), "Nested class in interface should be accepted (C# 8.0+)");
...
"#;
    let result = parse_interface_declaration(code);
    assert!(result.is_ok(), "Nested struct in interface should be accepted (C# 8.0+)");
    let (_, interface) = result.unwrap();
    assert_eq!(interface.body_declarations.len(), 1);
    match &interface.body_declarations[0] {
...
"#;
    let result = parse_interface_declaration(code);
    assert!(result.is_ok(), "Nested interface in interface should be accepted (C# 8.0+)");
    let (_, interface) = result.unwrap();
    assert_eq!(interface.body_declarations.len(), 1);
    match &interface.body_declarations[0] {
...
"#;
    let result = parse_interface_declaration(code);
    assert!(result.is_ok(), "Nested enum in interface should be accepted (C# 8.0+)");
    let (_, interface) = result.unwrap();
    assert_eq!(interface.body_declarations.len(), 1);
    match &interface.body_declarations[0] {
...
"#;
    let result = parse_interface_declaration(code);
    assert!(result.is_ok(), "Nested record in interface should be accepted (C# 8.0+)");
    let (_, interface) = result.unwrap();
    assert_eq!(interface.body_declarations.len(), 1);
    match &interface.body_declarations[0] {
...
"#;
    let result = parse_interface_declaration(code);
    assert!(result.is_ok(), "Interface with multiple valid members should be accepted");
    let (_, interface) = result.unwrap();
