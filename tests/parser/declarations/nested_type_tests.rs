...
fn test_parse_class_with_nested_class() {
    ...
    assert!(result.is_ok(), "Failed to parse class with nested class: {:?}", result);
    ...
    match &declaration.body_declarations[0] {
        ...
        _ => panic!("Expected nested class, got {:?}", declaration.body_declarations[0]),
    }
}
...
fn test_parse_class_with_nested_struct() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with nested struct: {:?}", result);
    ...
    match &declaration.body_declarations[0] {
        ...
        _ => panic!("Expected nested struct, got {:?}", declaration.body_declarations[0]),
    }
}
...
fn test_parse_class_with_nested_interface() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with nested interface: {:?}", result);
    ...
    match &declaration.body_declarations[0] {
        ...
        _ => panic!("Expected nested interface, got {:?}", declaration.body_declarations[0]),
    }
}
...
fn test_parse_class_with_nested_enum() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with nested enum: {:?}", result);
    ...
    match &declaration.body_declarations[0] {
        ...
        _ => panic!("Expected nested enum, got {:?}", declaration.body_declarations[0]),
    }
}
...
fn test_parse_class_with_nested_record() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with nested record: {:?}", result);
    ...
    match &declaration.body_declarations[0] {
        ...
        _ => panic!("Expected nested record, got {:?}", declaration.body_declarations[0]),
    }
}
...
fn test_parse_class_with_multiple_nested_types() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with multiple nested types: {:?}", result);
...
fn test_parse_class_with_deeply_nested_types() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with deeply nested types: {:?}", result);
...
fn test_parse_class_with_nested_types_and_members() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse class with mixed nested types and members: {:?}", result);
...
fn test_parse_nested_class_with_different_access_modifiers() {
    ...
    for (modifier_str, expected_modifier) in access_modifiers {
        ...
        let result = parse_class_declaration_helper(&code);
        assert!(result.is_ok(), "Failed to parse class with {} nested class: {:?}", modifier_str, result);
        
        let declaration = result.unwrap();
        match &declaration.body_declarations[0] {
            ClassBodyDeclaration::NestedClass(inner_class) => {
                assert!(inner_class.modifiers.contains(&expected_modifier), 
                       "Expected {} modifier for nested class", modifier_str);
            }
...
fn test_parse_nested_class_with_inheritance() {
    ...
    let result = parse_class_declaration_helper(code);
    assert!(result.is_ok(), "Failed to parse nested class with inheritance: {:?}", result);
