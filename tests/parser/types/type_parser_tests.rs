...
fn test_primitive_types() {
    assert_eq!(parse_test("int").unwrap(), Type::Primitive(PrimitiveType::Int));
    assert_eq!(parse_test("bool").unwrap(), Type::Primitive(PrimitiveType::Bool));
    assert_eq!(parse_test("string").unwrap(), Type::Primitive(PrimitiveType::String));
    assert_eq!(parse_test("void").unwrap(), Type::Primitive(PrimitiveType::Void));
}

#[test]
fn test_identifier_type() {
    assert_eq!(parse_test("MyClass").unwrap(), Type::Reference(Identifier { name: "MyClass".to_string() }));
}
...
fn test_array_type() {
    ...
    let qualified = Type::Array { 
        element_type: Box::new(Type::Reference(Identifier { name: "System.String".to_string() })), 
        rank: 1 
    };
...
fn test_nullable_type() {
    ...
    let nullable_class = Type::Nullable(Box::new(Type::Reference(
        Identifier { name: "DateTime".to_string() }
    )));
...
fn test_nullable_identifier() {
    let expected = Type::Nullable(Box::new(Type::Reference(Identifier { name: "MyClass".to_string() })));
