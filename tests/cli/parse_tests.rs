...
fn test_parse_command_creates_valid_json() -> Result<()> {
    ...
    for test_file in test_files {
        ...
        assert!(
            json_result.get("using_directives").is_some(),
            "JSON missing 'using_directives' field"
        );
        assert!(
            json_result.get("declarations").is_some(),
            "JSON missing 'declarations' field"
        );
    }
...
fn check_control_flow_features(json: &Value) -> Result<()> {
    ...
    assert!(
        json_str.contains("if") || json_str.contains("If"),
        "No if statements found"
    );
    assert!(
        json_str.contains("switch") || json_str.contains("Switch"),
        "No switch statements found"
    );
    assert!(
        json_str.contains("for") || json_str.contains("For"),
        "No for loops found"
    );
    assert!(
        json_str.contains("while") || json_str.contains("While"),
...
fn check_advanced_features(json: &Value) -> Result<()> {
    ...
    let json_str = json.to_string();
    assert!(
        json_str.contains("Generic") || json_str.contains("<"),
        "No generic types found"
    );
    assert!(
        json_str.contains("async") || json_str.contains("Async"),
        "No async features found"
    );
    assert!(
        json_str.contains("await") || json_str.contains("Await"),
