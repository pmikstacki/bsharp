...
fn test_tree_command_creates_valid_svg() -> Result<()> {
    ...
    for test_file in test_files {
        ...
        assert!(contents.contains("<svg"), "Output is missing SVG opening tag");
        assert!(contents.contains("</svg>"), "Output is missing SVG closing tag");
        
        // Check for visualization elements
        assert!(contents.contains("<rect"), "SVG doesn't contain any rectangle elements");
        assert!(contents.contains("<text"), "SVG doesn't contain any text elements");
    }
...
fn test_tree_command_handles_all_files() -> Result<()> {
    ...
    for test_file in test_files {
        ...
        assert!(result.is_ok(), "Failed to generate SVG for test file: {:?}", test_file);
        
        // Basic size check (SVG should not be empty)
        let metadata = fs::metadata(&output_path)?;
        assert!(metadata.len() > 100, "Generated SVG is too small: {} bytes", metadata.len());
    }
