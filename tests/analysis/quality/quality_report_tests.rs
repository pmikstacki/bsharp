...
fn test_quality_report_serialization() {
    ...
    let deserialized: QualityReport = serde_json::from_str(&json).expect("Failed to deserialize report");
...
fn test_quality_report_with_multiple_classes() {
    ...
    let first_class_report = report.class_reports.iter()
    ...
    let second_class_report = report.class_reports.iter()
