...
use bsharp::analysis::artifacts::dependencies::DependencyGraph;
...
"#;
    ...
    let graph = session.artifacts.get::<DependencyGraph>().expect("dependency graph missing");
    assert!(graph.node_count() >= 2, "expected at least 2 nodes (A and B/N)");
    assert!(graph.edge_count() >= 1, "expected at least 1 edge (A -> B.N method call)");
}
