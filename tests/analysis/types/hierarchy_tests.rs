...
use bsharp::syntax::nodes::declarations::ClassDeclaration;
...
fn class(name: &str, bases: Vec<&str>) -> ClassDeclaration {
    ClassDeclaration {
        ...
        base_types: bases.into_iter().map(|b| Type::Reference(ident(b))).collect(),
...
fn test_inheritance_hierarchy_and_is_derived_from() {
    ...
    assert!(hierarchy.get("A").map(|v| v.contains(&"B".to_string())).unwrap_or(true));
