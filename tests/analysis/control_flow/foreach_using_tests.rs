...
"#;
    ...
    let index = session.artifacts.get::<ControlFlowIndex>().expect("missing index");
    let stats = index.0.get("N.C::M").expect("missing method stats");
    assert!(stats.complexity >= 2, "expected cyclomatic complexity to account for foreach");
    assert!(stats.max_nesting >= 1, "expected nesting depth to include foreach body");
}
...
"#;
    ...
    let index = session.artifacts.get::<ControlFlowIndex>().expect("missing index");
    let stats = index.0.get("N.C::M").expect("missing method stats");
    assert!(stats.max_nesting >= 2, "expected nesting depth >= 2 for nested foreach");
}
...
"#;
    ...
    assert!(stats.max_nesting >= 2, "using contains if block -> nesting >= 2");
}
...
"#;
    ...
    let index = session.artifacts.get::<ControlFlowIndex>().expect("missing index");
    let stats = index.0.get("N.C::M").expect("missing method stats");
    assert!(stats.complexity >= 4, "foreach + using + if should grow complexity");
    assert!(stats.max_nesting >= 3, "nested foreach -> using -> if should yield depth >= 3");
}
