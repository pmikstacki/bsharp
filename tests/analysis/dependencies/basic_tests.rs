...
use bsharp::parser::nodes::declarations::{ClassDeclaration, InterfaceDeclaration, ClassBodyDeclaration, FieldDeclaration, MethodDeclaration};
use bsharp::parser::nodes::identifier::Identifier;
use bsharp::parser::nodes::types::{Type, Parameter};
...
fn test_basic_dependency_analysis() {
    ...
    assert!(dependencies.inherits_from.contains(&"BaseService".to_string()));
    assert!(dependencies.implements.contains(&"IUserService".to_string()));
    assert!(dependencies.field_dependencies.contains(&"IDatabase".to_string()));
    assert!(dependencies.method_dependencies.contains(&"User".to_string()));
    assert!(dependencies.method_dependencies.contains(&"UserId".to_string()));
...
fn test_dependency_inversion_analysis() {
    ...
    assert!(bad_violations.iter().any(|v| v.concrete_dependency == "SqlRepository"));
}
...
fn test_fan_in_fan_out_analysis() {
    ...
    let popular_module = ModuleDependencies {
        ...
        incoming_dependencies: vec!["ClientA".to_string(), "ClientB".to_string(), "ClientC".to_string(), "ClientD".to_string()],
    ...
    let complex_module = ModuleDependencies {
        ...
        outgoing_dependencies: vec!["ServiceA".to_string(), "ServiceB".to_string(), "ServiceC".to_string(), "ServiceD".to_string(), "ServiceE".to_string()],
        ..Default::default()
    };
...
fn test_dependency_impact_analysis() {
    ...
    let service_module = ModuleDependencies {
        ...
        outgoing_dependencies: vec!["Repository".to_string(), "EmailService".to_string(), "LoggingService".to_string()],
    ...
    assert_eq!(impact.total_affected_modules(), impact.directly_affected.len() + impact.transitively_affected.len());
}
...
fn test_interface_segregation_analysis() {
    ...
    let selective_client = ClassDeclaration {
        ...
        type_parameters: None,
        base_types: vec![
            Type::Reference(create_test_identifier("ILargeInterface")),
        ],
        body_declarations: Vec::new(), // Simplified
    };

    let segregation_issues = analyzer.analyze_interface_segregation(&large_interface, &selective_client);
...
fn test_dependency_metrics_comprehensive() {
    ...
    let business_layer = ModuleDependencies {
        ...
        outgoing_dependencies: vec!["DataLayer".to_string(), "EmailService".to_string(), "LoggingService".to_string()],
