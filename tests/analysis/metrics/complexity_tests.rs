...
fn test_complexity_threshold_analysis() {
    ...
    let high_complexity_body = Statement::Block(vec![
        Statement::If(Box::new(IfStatement {
            condition: Expression::Literal(Literal::Boolean(true)),
            consequence: Box::new(Statement::If(Box::new(IfStatement {
                condition: Expression::Literal(Literal::Boolean(true)),
...
fn test_complexity_comparison() {
    ...
    // Simple method
    let simple_method = create_test_method("Simple", Some(Statement::Block(vec![
        Statement::Expression(Expression::Literal(Literal::Integer(1)))
    ])));
    
    // Complex method
    let complex_method = create_test_method("Complex", Some(Statement::Block(vec![
        Statement::If(Box::new(IfStatement {
            condition: Expression::Literal(Literal::Boolean(true)),
            consequence: Box::new(Statement::For(Box::new(ForStatement {
                initializer: None,
