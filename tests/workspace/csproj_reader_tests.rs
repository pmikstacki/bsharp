...
use bsharp::workspace::model::{Language, ProjectFileKind};
...
fn sdk_style_defaults_collect_cs_files_and_exclude_common_dirs() {
    ...
    let paths: Vec<_> = prj.files.iter().map(|f| f.path.canonicalize().unwrap()).collect();

    assert!(paths.contains(&f1.canonicalize().unwrap()));
    assert!(paths.contains(&f2.canonicalize().unwrap()));
    assert!(!paths.contains(&bin_cs.canonicalize().unwrap()), "bin/** should be excluded");
    assert!(!paths.contains(&obj_cs.canonicalize().unwrap()), "obj/** should be excluded");
...
fn compile_include_and_remove_patterns_are_applied() {
    ...
    let csproj_path = root.join("App.csproj");
    write(&csproj_path, r#"<Project Sdk="Microsoft.NET.Sdk">
  <ItemGroup>
    ...
    let prj = CsprojReader::read(&csproj_path).expect("csproj parse");
    let paths: Vec<_> = prj.files.iter().map(|f| f.path.file_name().unwrap().to_string_lossy().to_string()).collect();
...
fn project_reference_is_resolved_to_absolute_path_or_warns() {
  ...
  <ItemGroup>
    ...
    let prj = CsprojReader::read(&csproj_path).expect("csproj parse");
    assert!(prj.project_references.iter().any(|p| p.canonicalize().unwrap() == ref_csproj.canonicalize().unwrap()));
}
